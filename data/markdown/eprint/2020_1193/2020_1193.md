...
RISC-V is an open instruction set architecture (ISA) published in 2011 [PW17]. It has attracted considerable attention in both academia and industry owing to its remarkable features of no license fee, expandability with reduced wasteful functions in existing ISAs, and flexibility with m odular e xtensions [ Fou19]. O wing t o i ts fl exibility, it ca n be used in a wide variety of applications from low-end embedded devices running a bare-metal program to high-end servers running the Linux operating system (OS).
