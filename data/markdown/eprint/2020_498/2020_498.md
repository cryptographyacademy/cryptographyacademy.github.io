...
First of all, due to the technical limitations of the Bitcoin Script, it is not possible to realize such a threshold custody via multisignatures (it would be also overly expensive to do so due to the large transaction size). Secondly, even though several universal threshold schemes for ECDSA already exist (see [LN18, GG18, CCL<sup>+</sup>20]) they do not allow for the construction of such a system, due to a specific way these protocols operate. More specifically, each of the protocols [LN18, GG18, CCL<sup>+</sup>20] generates a t-threshold sharing of a secret key  $x \in \mathbb{Z}_q$ , then, given a message  $m \in \mathbb{Z}_q$  and a subset S of exactly t out of the N parties, these t parties run a Sign(m) protocol that results in one of:
...
where " · " in the left hand side denotes coordinate-wise multiplication in  $G^2$ . Consequently also, given a commitment E(x) and a scalar  $\alpha \in \mathbb{Z}_q$ , we can compute (without the knowledge of  $x \in \mathbb{Z}_q$ ) a commitment  $E(\alpha x)$  by fast exponentiation.

Non-Malleable Commitments. Except for ElGamal commitments, we also need a different kind of commitments that allow the parties to commit to any piece of data and guarantee non-malleability, i.e., roughly speaking, given a commitment C(d) to a piece of data d, it is computationally infeasible to generate a commitment C(d') for d' that is in any non-trivial way related to d. A reader interested in the formalization of this concept we refer the reader to the discussion of the  $\mathcal{F}_{\text{Com}}$  functionality in Section C. We offer a simple, practical implementation of this component that is based on a hash function  $\text{Hash}(\cdot)$  and can be formalized in the Random Oracle model.

The commitment functionality consists of two procedures:  $Commit(\cdot)$  and  $Decommit(\cdot)$ 
...
The presignature for party  $k \in [N]$  consists, for each  $a \in \{k, \rho, \eta, \tau\}$  of shares  $a_k \in \mathbb{Z}_q$  and ElGamal commitments  $\mathsf{E}(a_l) \in G^2$  for each  $l \in [N]$ . Thus 4 shares and 4N commitments in total. After generating such a presignature by calling the  $\mathcal{P}_{\mathrm{APresign}}$  protocol, the resulting additively shared variables are then turned into threshold-t shared variables by executing the  $\mathcal{P}_{\mathrm{Reshare}}(t)$  protocol. Since the ElGamal commitments and zero-knowledge proofs are carried through all the stages of presigning, if the protocol is not aborted, then all parties necessarily have correct presignatures at the end of this subprotocol.
...
We proceed to formulate and discuss some auxiliary subprotocols that are used in  $\mathcal{P}_{ECDSA}$  in Section 4.1. These include 3 subprotocols (see Auxiliary subprotocols of  $\mathcal{P}_{ECDSA}(t)$ )–ExpRevealThreshold, Lin and Reveal—that were omitted for clarity from the previous section, as well as the  $\mathcal{P}_{GER}$  protocol that is used both in  $\mathcal{P}_{ECDSA}$  and in  $\mathcal{P}_{APresign}$  (see Section 5.1). We start by a brief description of ExpRevealThreshold, Lin and Reveal, followed by pseudocode.
...
The purpose of the  $\mathcal{P}_{GER}$  protocol is to generate a uniformly random scalar  $a \in \mathbb{Z}_q$  and reveal  $g^a$  to the public. This protocol is used to generate the shared ECDSA private key x as well as the ElGamal private key d (so that  $h := q^d$  is the public key used for ElGamal commitments).
...
where  $\mathtt{E}(\widehat{a}_l)$  and  $\mathtt{E}\Big(\widehat{b}_l\Big)$  are public.
...
- In the  $\mathsf{Mult}(a,b,c)$  subprotocol, the parties hold an additive sharing of a and b and would like to obtain an additive sharing of  $c := a \cdot b$ . The idea of this subprotocol follows [LN18]: first the parties involve in a multiplication protocol  $\mathcal{P}_{\mathsf{PrivMult}}$  (we discuss it in Section 5.2) that guarantees privacy (nothing is leaked to the adversary) but does not guarantee correctness (the adversary might inject errors in the output). Let the output shares of  $\mathcal{P}_{\mathsf{PrivMult}}$  be  $(c_1, c_2, \ldots, c_N)$ , the goal is to check that  $\sum_{l \in [N]} c_l = a \cdot b$ . For this, the parties make use of the ElGamal commitments to the shares of a, b and c. First, they participate in a protocol whose result is an ElGamal commitment to the product  $\mathsf{E}(ab)$  and subsequently, the parties compute  $\mathsf{E}(c)$ , where  $c := \sum_{l \in [N]} c_l$ . It is now not hard to see that
...
- 3. Choose a random  $r_k \in \mathbb{Z}_q$ , and publish  $\mathbf{E}(c_k; r_k)$  together with a ZKPoK of  $c_k$  and  $r_k$  (R<sub>EGKnow</sub>).
...
- 6. Run the  $\mathcal{P}_{\text{CheckDH}}.\text{Query}(u,v)$  protocol with input  $(u,v) := \mathbf{E}(a \cdot b) \cdot \mathbf{E}(c)^{-1}$ ; continue in case the output is accept and abort in case the output is reject.

# <span id="page-13-0"></span>5.2 Private Multiplication Protocol $\mathcal{P}_{\text{PrivMult}}$
...
$$\left(\sum_{l\in[N]} a_l\right) \cdot \left(\sum_{l\in[N]} b_l\right) = \sum_{l\in[N]} a_l b_l + \sum_{l,k\in[N],l\neq k} a_l b_k.$$
...
**Private input:** Each party  $l \in [N]$  holds  $a_l, \tilde{r}_l \in \mathbb{Z}_q$  such that  $\mathbf{E}(a_l, \tilde{r}_l) = c_l$ .
...
- 3. Compute commitments to coefficients of  $f_k$ :  $C_k := \{ \mathbb{E}(f_{k,\iota}, r_{k,\iota}) : \iota = 0, 1, 2, \dots, t-1 \}$  together with  $\Pi_k$ : a ZKPoK of committed values ( $\mathbb{R}_{EGKnow}$ ) and  $\pi_l$ : a ZKP that  $\mathbb{E}(f_{k,0}, r_{k,0})$  commits to the same value as  $c_k$  ( $\mathbb{R}_{EGRefresh}$ ).
...
- 7. For each  $l \in [N]$  recommit to  $f_k(l)$  by sampling a fresh randomizing element  $r'_{k \to l} \in \mathbb{Z}_q$  and publishing  $\mathsf{E}(f_k(l), r_{k \to l})$  along with a zero-knowledge proof that it agrees with the previous commitment ( $\mathsf{R}_{\mathsf{EGRefresh}}$ ).
...
- <span id="page-18-4"></span><span id="page-18-3"></span><span id="page-18-1"></span><span id="page-18-0"></span>[BDOZ11] Rikke Bendlin, Ivan Damgård, Claudio Orlandi, and Sarah Zakarias. Semi-homomorphic encryption and multiparty computation. In Kenneth G. Paterson, editor, Advances in Cryptology - EUROCRYPT 2011 - 30th Annual International Conference on the Theory and Applications of Cryptographic Techniques, Tallinn, Estonia, May 15-19, 2011. Proceedings, volume 6632 of Lecture Notes in Computer Science, pages 169–188. Springer, 2011.
  ...
  - [BFM88] Manuel Blum, Paul Feldman, and Silvio Micali. Non-interactive zero-knowledge and its applications (extended abstract). In Janos Simon, editor, Proceedings of the 20th Annual ACM
...
- <span id="page-20-12"></span><span id="page-20-3"></span><span id="page-20-0"></span>[DKLS18] Jack Doerner, Yashvanth Kondi, Eysa Lee, and Abhi Shelat. Secure multi-party threshold ecdsa from ecdsa assumptions. In Oakland S&P'2018, 2018.
...
- <span id="page-20-11"></span><span id="page-20-8"></span><span id="page-20-6"></span><span id="page-20-5"></span>[FLM86] Michael J. Fischer, Nancy A. Lynch, and Michael Merritt. Easy impossibility proofs for distributed consensus problems. Distributed Comput., 1(1):26–39, 1986.
...
- <span id="page-20-10"></span><span id="page-20-7"></span><span id="page-20-4"></span>[GGN16] Rosario Gennaro, Steven Goldfeder, and Arvind Narayanan. Threshold-optimal DSA/ECDSA signatures and an application to bitcoin wallet security. In Mark Manulis, Ahmad-Reza Sadeghi, and Steve Schneider, editors, Applied Cryptography and Network Security - 14th International Conference, ACNS 2016, Guildford, UK, June 19-22, 2016. Proceedings, volume 9696 of Lecture Notes in Computer Science, pages 156–174. Springer, 2016.
...
# <span id="page-25-0"></span>B.2 Analysis of $\mathcal{P}_{\mathrm{MtA}}$
...
#### Functionality $\mathcal{F}_{MtA}$ ( $\mathcal{A}$ controls Alice)
...
# Functionality $\mathcal{F}_{2\mathsf{PMul}}^1$

**Private input:** Alice holds  $\alpha \in \mathbb{Z}_q$  and Bob holds  $\beta \in \mathbb{Z}_q$ .

- 1. Bob sends  $\beta$  to  $\mathcal{F}^1_{2\mathsf{PMul}}$ .
- 2.  $\mathcal{F}_{2PMul}^1$  samples  $t_A$  uniformly at random and sends it to Alice.
...
$$\delta := \left(\sum_i a_i\right) \left(\sum_i b_i\right) - \sum_i c_i.$$
...
We analyze the  $\mathcal{P}_{PrivMult}$  protocol in the  $\mathcal{F}_{MtA}$ -hybrid model. To this end, we replace each execution of  $\mathcal{P}_{MtA}$  by a call to the  $\mathcal{F}_{MtA}$  functionality. It will be convenient also to denote the call to  $\mathcal{F}_{MtA}$  corresponding to the instance where party  $k \in [N]$  acts as Alice and party  $l \in [N]$  acts as Bob by  $\mathcal{F}_{MtA}^{k \to l}$ . Thus, in particular, in step 1. of  $\mathcal{P}_{PrivMult}$  party  $k \in [N]$  interacts with  $\mathcal{F}_{MtA}^{k \to l}$  and in step 2. party k interacts with  $\mathcal{F}_{MtA}^{l \to k}$ .
...
- 3. The simulator S sends  $\{a_{i,j}, b_{i,j}\}_{i \in I, j \in J}$  to  $\mathcal{F}_{\text{PrivMult}}$ .
...
$$c_{j} = a_{j}b_{j} + \sum_{j' \in J, j' \neq j} (\alpha_{j \to j'} + \beta_{j' \to j}) + \sum_{i} \delta_{i,j}$$

$$+ \sum_{i} (b_{j}a_{i,j} + a_{j}b_{i,j}) - \sum_{i} \alpha_{i \to j} - \sum_{i} \beta_{j \to i}$$

$$= \tau_{j} + \sum_{i} \delta_{i,j} + \sum_{i} (b_{j}a_{i,j} + a_{j}b_{i,j}) - \sum_{i} \alpha_{i \to j} - \sum_{i} \beta_{j \to i}$$

{28}------------------------------------------------

Where we denoted  $\tau_j := a_j b_j + \sum_{j' \in J, j' \neq j} (\alpha_{j \to j'} + \beta_{j' \to j})$ . Now, note that  $\alpha_{j \to j'}$  and  $\beta_{j' \to j}$  come from interactions between two honest parties via  $\mathcal{F}_{MtA}$ , and, thus, are uniformly random subject to  $\alpha_{j \to j'} + \beta_{j \to j'} = a_j b_{j'}$ , hence
...
# B.4 Analysis of $\mathcal{P}_{\text{CheckDH}}$
...
Subsequently, we show that  $\mathcal{P}_{\text{CheckDH}}$  securely computes the  $\mathcal{F}_{\text{CheckDH}}$  functionality. To this end, we first reformulate the  $\mathcal{P}_{\text{CheckDH}}$  protocol in the  $(\mathcal{F}_{\text{ZK}}, \mathcal{F}_{\text{ComZK}})$ -hybrid model, below we list the necessary changes:

- In step 1 of Init the parties call  $\mathcal{F}_{ZK}(R_{DLog})$
- In steps 1,2,3 of Query the parties interact with  $\mathcal{F}_{ComZK}(R_{Rerand})$
...
- 5. S simulates  $\mathcal{F}_{\text{ComZK}}$  to decommit  $(u_j, v_j)$  for  $j \in J$  to the adversary A.
...
- 1. Receive from  $\mathcal{A}$  shares  $\{a_i\}_{i\in I}\subseteq \mathbb{Z}_q$ .
...
#### Functionality $\mathcal{F}_{Reshare}(t)$
...
- 2. Verify that  $\mathbb{E}(a_l, r'_l) = c_l$  for each  $l \in [N]$  and abort if this is not the case.
...
- 7. Sample  $r_j \in \mathbb{Z}_q$  for every  $j \in J$  and publish  $\mathbf{E}(\widehat{a}_j; r_j)$ .
- 8. Receive  $\{r_i\}_{i\in I}$  from  $\mathcal{A}$  and publish  $\mathbb{E}(\widehat{a}_i; r_i)$ .
...
<span id="page-31-0"></span>The above specify |I| + 1 linearly independent constraints. To specify the polynomial  $f_{j_0}$  completely, S also chooses  $f_{j_0,\iota}$  uniformly at random, for  $\iota = 1, 2, \ldots, t - |I| - 1$ .
...
- 8.  $\mathcal{S}$  computes the commitments to the evaluations of all polynomials as in the protocol.
...
- 10. For each  $j \in J$  and  $i \in I$ ,  $\mathcal{S}$  acting as j sends the share  $(f_j(i), r'_{i \to i})$  to  $\mathcal{A}$ .
...
- 4.  $\mathcal{S}$  intercepts the values  $\widetilde{c}_i$  for  $i \in I$  that  $\mathcal{A}$  sends to  $\mathcal{F}_{ZK}$
...
$$\sum_{i \in I} \widetilde{a}_{i,j} = \sum_{i \in I} a_i \quad \text{for every } j \in J, \tag{3}$$
...
$$\sum_{i \in I} \widetilde{a}_{i,j} = \sum_{i \in I} a_i \quad \text{for every } j \in J, \tag{5}$$
...
We now proceed to the proof that the output distribution generated by an execution with  $\mathcal{S}$  in the ideal world is computationally indistinguishable from the distribution generated by  $\mathcal{A}$  in a real-world execution. We proceed via a sequence of games:
...
$$\sum_{j \in I} b_j \widetilde{\delta}_j' = 0, \tag{13}$$
...
**Game 4:** The simulator S instead of using random pairs of group elements, generates correct commitments whenever publishing commitments as an honest party. To do so, it uses honest parties' shares obtained from  $\mathcal{F}_{APresign}$  (due to the modification introduced in Game 1).
...
- 1. If (g, h, u, v) form a Diffie-Hellman tuple (that is, if  $u^r = v$  and  $g^r = h$  for some r) then Rerand(u, v) = (u', v') is a random tuple under the constraint that (g, h, u', v') forms a Diffie-Hellman tuple.
...
- (b) When publishing the commitment to  $c_j$  during the execution of Mult(a, b, c), the simulator knows  $c_j$  from the functionality  $\mathcal{F}_{PrivMult}$ .
...
- In step 1. of Presign(), instead of running  $\mathcal{P}_{APresign}.Setup()$  the parties interact with  $\mathcal{F}_{APresign}$  by invoking Gen().
...
*Proof.* Let  $\mathcal{A}$  be any adversary for  $\mathcal{P}_{\text{ECDSA}}(t)$ . We start by describing a suitable simulator  $\mathcal{S}$ . Simulating GenKey()

- 1.  $\mathcal{S}$  acting as the  $\mathcal{F}_{GER}$  functionality receives shares  $\{x_i\}_{i\in I}$  from  $\mathcal{A}$ .
- 2. S sends  $\{x_i\}_{i\in I}$  to the  $\mathcal{F}_{\text{wECDSA}}$  functionality and receives back  $\{g^{x_j}\}_{j\in J}$ .
- 3.  $\mathcal{S}$  simulates the  $\mathcal{F}_{GER}$  functionality by sending  $\{g^{x_j}\}_{j\in J}$  to  $\mathcal{A}$ . If  $\mathcal{A}$  chooses to abort,  $\mathcal{S}$  aborts as well.
...
- 6. S simulates  $\mathcal{F}_{Reshare}(t)$  for variables  $a \in \{k, \rho, \eta, \tau\}$  as follows:
  - (a) S fetches the adversarial shares  $\{\widehat{a}_i\}_{i\in I}$  generated by S in step (1) of the simulation of Presign to A. In case A aborts, S aborts as well.
...
- 2. S simulates the ExpRevealThreshold(k) subprotocol call:
  - (a) S receives  $\{g^{\hat{k}_j}\}_{j\in J}$  from the  $\mathcal{F}_{\text{wECDSA}}$  functionality.
...
- 3. S simulates the Reveal $(\tau)$  subprotocol call:
  - (a) S receives  $\{\hat{\tau}_j\}_{j\in J}$  from the  $\mathcal{F}_{\text{wECDSA}}$  functionality
  - (b)  $\mathcal{S}$  acting as honest parties publishes  $\widehat{\tau}_j$  for  $j \in J$  and simulates  $\mathcal{F}_{ZK}$  sending accept to  $\mathcal{A}$ .
...
- 3.  $\mathcal{S}$  generates  $k, \rho$  uniformly at random in  $\mathbb{Z}_q$  and computes  $\eta, \tau$ ,
- 4. S samples  $a_J \in \mathbb{Z}_q^J$  uniformly at random subject to the constraint  $\sum_{j \in J} a_j = a \sum_{i \in I} a_i$  for all  $a \in \{k, \rho, \eta, \tau\}$ .
...
We note first that the only impact  $\mathcal{A}$  has is picking additive adversarial shares in Step (1) (in both of the above lists). Further, steps (1) and (2) are in both cases the same, showing that the distribution of adversarial (additive and threshold) shares is in both case the same.
...
Finally, let us argue that the mere fact that the distributions of shares are the same implies indistinguishability between Game 3 and Game 4. Firstly: the only part of this distribution  $\mathcal{A}$  learns during Presign are the adversarial shares  $k_I$ ,  $\rho_I$ ,  $\eta_I$ ,  $\tau_I$  (which  $\mathcal{A}$  chooses) and  $\widehat{k}_I$ ,  $\widehat{\rho}_I$ ,  $\widehat{\eta}_I$ ,  $\widehat{\tau}_I$  (which are chosen independently, uniformly at random by  $\mathcal{S}$ ). Thus, during Presign() the adversary  $\mathcal{A}$  cannot make a distinction. On the other hand, during Sign(·) the distribution over  $\mathcal{A}$ 's views depends solely on the shares  $\widehat{k}_I$ ,  $\widehat{\rho}_I$ ,  $\widehat{\eta}_I$ ,  $\widehat{\tau}_I$  and  $\widehat{k}_J$ ,  $\widehat{\rho}_J$ ,  $\widehat{\eta}_J$ ,  $\widehat{\tau}_J$  generated in Presign(), hence we conclude indistinguishability between Game 3 and Game 4.

Game 5: In this game, S does not generate  $k, \rho, \eta, \tau$  and the additive shares explicitly by itself, instead it honestly simulates the execution of  $\mathcal{F}_{APresign}$ , with the only modification that at the end, S generates random pairs in  $G^2$  instead of ElGamal commitments to shares of honest parties.
...
- 3. Let J' be any subset of honest parties that satisfies |J'| + |I| = t 1. The simulator randomly samples the shares  $\hat{k}_j \in \mathbb{Z}_q$  for  $j \in J'$ . Now the values of the polynomial  $f^{(k)} \in \mathbb{Z}_q[X]$  that is used to share k are specified at t points
  ...
  - $f^{(k)}(0) = k$ , not knowing  $k = \log_g(R)$  is not a problem here since it will never be used in computations

Hence f is uniquely specified, as a polynomial of degree at most t-1. It follows that the share  $\widehat{k}_j$  (i.e., the value  $f^{(k)}(j)$ ) for  $j \in J \setminus J'$  is now uniquely specified as
...
- 7. S computes  $\widehat{\tau}_j := f^{(\tau)}(j)$  for  $j \in J$  and sends  $\{\widehat{\tau}_j\}_{j \in J}$  to  $\mathcal{A}$  and receives a set I'' as a response.
...
- 9. S samples a random polynomial  $f^{(s)} \in \mathbb{Z}_q[X]$  of degree  $\leq t-1$  subject to the constraints:
  - $f^{(s)}(i) = \widehat{s}_i$ , where  $\widehat{s}_i := \widehat{\eta}_i r + \widehat{\rho}_i m$ ,
  - $f^{(s)}(0) = s$ .
- 10. S computes  $\hat{s}_j := f^{(s)}(j)$  for  $j \in J$  and sends  $\{\hat{s}_j\}_{j \in J}$  to A and receives a set I''' as a response.
- 11. S internally publishes the shares  $\{\widehat{s}_l\}$  for  $l \in J \cup I'''$ . In case when  $|J| + |I'''| \le t 1$ , S externally terminates  $\mathsf{Sign}(\cdot)$  with fail and declares  $F := I \setminus I'''$  as the "faulty parties".
- 12.  $\mathcal{S}$  outputs whatever  $\mathcal{A}$  outputs and halts.

We now show that the output distribution of  $\mathcal{A}$  is the same as the output distribution of  $\mathcal{S}$  (up to events of probability  $O(q^{-1})$ ). To start, let us see that the distribution of  $\{x_l\}_{l\in[N]}$  is the same when generated inside  $\mathcal{F}_{wECDSA}$  and in the simulation by  $\mathcal{S}$ :
...
for two corresponding calls of  $\mathsf{Presign}()$  and  $\mathsf{Sign}(m)$  are the same when generated inside  $\mathcal{F}_{\text{wECDSA}}$  and in the simulation with  $\mathcal{S}$ . We note that the tuple  $(\widehat{k}_J, \widehat{\tau}_J, \widehat{s}_J)$  in (14) is all that the adversary can see in an execution of  $\mathcal{F}_{\text{wECDSA}}$  and thus all that needs to be considered in the analysis; in particular, the shares  $\widehat{\eta}_J$  and  $\widehat{\rho}_J$  are never revealed to  $\mathcal{A}$ .
...
When analyzing  $\widehat{\tau}_J$  the situation is quite simple, since in both scenarios these shares come from the same distribution, i.e., a polynomial  $f^{(\tau)}$  is chosen uniformly at random, subject to  $f^{(\tau)}(i) = \widehat{\tau}_i$  for  $i \in I$ , the values  $\widehat{\tau}_J$  are simply evaluations of  $f^{(\tau)}$  over all  $j \in J$ .
...
- $\mathcal{F}_{\text{wECDSA}}$  computes  $f^{(s)} = \tau^{-1}(rf^{(\eta)} + mf^{(\rho)})$ . The shares  $\widehat{s}_J$  are now given as  $f^{(s)}(j)$  for  $j \in J$ .

#### 2. In the simulation:

- $\mathcal{A}$  generates  $(\widehat{k}_I, \widehat{\rho}_I, \widehat{\eta}_I, \widehat{\tau}_I)$
- S given (r, s) from  $\mathcal{F}_{ECDSA}$  generates the polynomial  $f^{(k)}$  which determines k, R and r.
- The polynomial  $f^{(\tau)}$  is generated by  $\mathcal{S}$  and the value  $\tau$  and shares  $\tau_J$  are determined.
- S computes a polynomial  $f^{(s)}$  by setting  $f^{(s)}(i) = \hat{s}_i$ ,  $f^{(s)}(0) = s$  and choosing it uniformly at random subject to these constraints. The shares  $\hat{s}_J$  are now given as  $f^{(s)}(j)$  for  $j \in J$ .
...
# C.1 The $\mathcal{F}_{\mathrm{ZK}}$ Functionality
...
The above standard functionality simply receives both the public and private data (x, w) from a particular party  $l \in [N]$  corresponding to a unique identifier<sup>9</sup> id, and broadcasts x along with the information whether  $(x, w) \in R$  to all the parties. The  $\mathcal{F}_{ZK}(R)$  functionality, for all the relations R that are of use in our protocol has efficient non-interactive realizations. We refer to [LN18] where all of them are described in detail. Roughly speaking, each of the relations R has a corresponding  $\Sigma$ -protocol which via a standard application of Fiat-Shamir heuristic [FS87] can be compiled into a non-interactive zero-knowledge proof of knowledge.
...
- 4. Compute "honest shares" as follows:
  ...
  - (b) Sample a uniformly random polynomial  $f^{(\rho)} \in \mathbb{Z}_q[X]$  of degree  $\leq t-1$  subject to constraints  $f^{(\rho)}(i) = \widehat{\rho}_i$  for  $i \in I$  and compute  $\widehat{\rho}_i = f^{(\rho)}(j)$  for  $j \in J$ .
  ...
  - (e) Sample a uniformly random polynomial  $f^{(\tau)} \in \mathbb{Z}_q[X]$  of degree  $\leq t-1$  subject to constraints  $f^{(\tau)}(i) = \widehat{\tau}_i$  for  $i \in I$  and  $f^{(\tau)}(0) = k\rho$ . Compute  $\widehat{\tau}_i = f^{(\tau)}(j)$  for  $j \in J$ .
...
- 8. Send  $\{\widehat{\tau}_j\}_{j\in J}$  to  $\mathcal{A}$  and receive another subset  $I''\subseteq I$  from  $\mathcal{A}$
...
- 13. Publish  $\hat{s}_l$  for  $l \in J \cup I'''$
