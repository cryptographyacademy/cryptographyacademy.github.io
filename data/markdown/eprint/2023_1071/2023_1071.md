...
$$\varepsilon_{\mathsf{rbr}}^{\mathsf{bFRI}}(\mathbb{F}, \rho, \delta, n, \ell, t) = \max\{O((2^{2n})/(\rho^{3/2}|\mathbb{F}|)), (1-\delta)^{\ell}\}.$$
...
<span id="page-3-2"></span>**Corollary 1.4** (Informally Stated; see Corollary 4.4). For finite field  $\mathbb{F}$ , evaluation domain  $L \subset \mathbb{F}$  of size  $2^n$ , constants  $\rho \in (0,1)$ ,  $\delta \in (0,1-\sqrt{\rho})$ , and positive integers  $\ell$ ,  $\ell$ , given a random oracle with  $\ell$ -bits of output and query bound  $\ell$   $\ell$  2, compiling Batched FRI with the BCS transformation yields a non-interactive argument in the random oracle model with adaptive soundness error and knowledge error
...
$$\varepsilon_1 = O(2^{2n}/(\rho^{3/2}|\mathbb{F}|))$$
  $\varepsilon_2 = O((2^n \cdot n\sqrt{\rho})/|\mathbb{F}|)$   $\varepsilon_3 = (1-\delta)^{\ell}.$ 
...
$$\delta\text{-correlated IOP} \xrightarrow{\begin{array}{c} \text{IOPP for} \\ \delta\text{-correlation} \end{array}} \text{IOP} \xrightarrow{\begin{array}{c} \text{BCS} \\ \text{Transformation} \end{array}} \text{SNARG/SNARK}$$
...
$$\varepsilon_{\mathsf{rbr}}^{\mathsf{OPlonky},\mathcal{O}}(\mathbb{F},\mathbb{K},D,n,k,r,s,t,u,d,\rho,\eta) = \max\left\{O\left(\left(\frac{n(r+u)}{|\mathbb{F}|}\right)^t\right),O\left(\left(\frac{k+st}{|\mathbb{F}|}\right)^t\right),\frac{n\cdot \max\{u+1,d\}}{|\mathbb{K}\setminus D|}\right\}.$$
...
- In the last round of interaction, the verifier sends a field element 3 ∈ K \ D to the prover, and the prover replies with values

<span id="page-12-1"></span>
$$\{g_i(k_{i,j}\mathfrak{z}) \mid i \in [m], j \in [n_i]\}$$
 (2)
...
$$\left\{ \frac{g_i(X) - g_i(k_{i,j}\mathfrak{z})}{X - \mathfrak{z}} \mid i \in [m], j \in [n_i] \right\}$$
 (3)
...
must have the form  $\nu = (\tau', m', c)$ , where  $\tau'$  is a *i*-round low-degree-partial transcript that is  $\delta$ -correlated with  $\tau$ . In particular,  $\tau'$  is doomed in  $\Pi_0$ .
...
$$u_{i,j}(X) \coloneqq q_{i,j}(X)(X-k_{i,j}\mathfrak{z}) + g_i(k_{i,j}\mathfrak{z})$$
...
We have seen so far that  $g_i(X)$  agrees with the polynomial  $u_i(X)$  on S, for all  $i \in [m]$ , and that  $g_i(k_{i,j}\mathfrak{z}) = u_i(k_{i,j}\mathfrak{z})$ , for all i, j. Thus  $\tau' = \tau(u_1, \ldots, u_m)$  is a low-degree partial transcript that is  $\delta$ -correlated with  $\tau$ . Since  $\tau$  is a doomed transcript and quotients have  $\delta$ -correlated agreement in RS[F, D, d], we must have that  $\tau'$  is doomed in  $\Pi_0$ . Note that  $\tau'$  is a complete transcript, and so  $\Pi_0$ 's verifier rejects it. Clearly,  $\tau'$  passes the 0-correlated agreement check of  $\Pi_0$ 's verifier. Hence the first check of the verifier fails, i.e. the values  $\{u_i(k_{i,j}\mathfrak{z}) \mid i \in [m], j \in [n_i]\}$  do not satisfy the polynomial equations they are meant to satisfy. However, these values coincide with  $\{g_i(k_{i,j}\mathfrak{z}) \mid i \in [m], j \in [n_i]\}$ , and so the verifier of  $\Pi_\delta$  rejects  $\tau$  because of the same reason: the values do not satisfy the polynomial equations they need to satisfy. This proofs that  $\Pi_\delta$  has the claimed RBR soundness error.
...
1. **Round 1.** The prover sends r polynomials  $a_1(X), \ldots, a_r(X)$  of degree < n to the verifier as oracles. Each of these polynomials is the result of interpolating the columns of  $\mathbb W$  over a multiplicative subgroup H of  $\mathbb F$  or order n. The verifier then replies with some random challenges.
...
$$Q_j \coloneqq P_j(a_1(X), \dots, a_r(X))$$
...
and proving that d(X) vanishes in H. To do so, the prover sends the verifier oracle access to the polynomial  $q(X) := d(X)/\mathsf{Z}_H(X)$ , where  $\mathsf{Z}_H(X)$  is the vanishing polynomial of H.
...
- Now suppose that, right at the end of Round 2, the polynomial d(X) does not vanish on  $Z_H(X)$ . Then we set the state to "doomed".
  - In this case, the probability of starting Round 2 in a doomed state and finishing it in a non-doomed state is at most, roughly,  $(k+s)/|\mathbb{F}|$ . This is deduced by taking an arbitrary  $x \in H$  and looking at the equality d(x) = 0 as a polynomial equation on  $\alpha$ . This equation either has degree  $\alpha + s \in H$ , or it is identically zero. However, we see that if Round 2 started in a doomed state, then R(x) = 0 is not identically zero for at least one  $x \in H$ . Hence, there are at most  $\alpha + s \in H$  distinct values of  $\alpha + s \in H$ .
- Finally, suppose that right at the end of Round 3, one has  $q(\mathfrak{z})Z_H(\mathfrak{z}) \neq d(\mathfrak{z})$ . Then we set the state to "doomed". On this occasion, the probability of ending Round 3 in a non-doomed state, if the state was previously doomed, is at most, roughly,  $\max_j \{\deg P_j\} \cdot n/|\mathbb{F}|.^8$  This is because either  $q(X)Z_H(X) d(X)$  is the zero polynomial (as it should be), or it is a polynomial of degree  $\max_j \{\deg P_j\}n$ , and  $\mathfrak{z}$  is a root to it. We then see that if the protocol is in a doomed state when Round 3 starts, then  $q(X)Z_H(X) d(X)$  is a nonzero polynomial. Notice as well that if the protocol ends in a doomed state as per our definition, then the verifier rejects.
...
We begin by discussing the ethSTARK protocol  $\Pi_{ethSTARK}$  [Sta23], which is a close variation of the DEEP-ALI protocol [BGKS20]. We briefly provide a rough overview of the protocol; see [Sta23] for complete details.
...
$$\sum_{i \in I} (r_i + r_i' X^{c_i}) \frac{Q_i(X, f_1(g_{i,1} X), \dots, f_m(g_{i,m} X))}{Z_{H_i}(X)} = \sum_{j=1}^k X^{j-1} q_j(X^k)$$
 (6)
...
$$\left\{ f_1(g_{i,j}\mathfrak{z}), \dots, f_m(g_{i,j}\mathfrak{z}), q_1(\mathfrak{z}), \dots, q_k(\mathfrak{z}) \mid i \in I, j \in [m] \right\}. \tag{7}$$
...
It follows that, in its 0-correlated form,  $\Pi_{ethSTARK}$  has RBR soundness and RBR knowledge soundness with error
...
$$\varepsilon \coloneqq \max\{\ell/|\mathbb{K}|, \ell d'/|S|, \varepsilon_{\mathsf{rbr}}^{\mathsf{bFRI}}\}$$
...
For any positive integer m, we let [m] denote the set  $\{1,\ldots,m\}$ . Throughout this work, we let  $\mathbb{F}$  denote a finite field of prime p elements and let  $\mathbb{F}^* := \mathbb{F} \setminus \{0\}$  denote the multiplicative group of  $\mathbb{F}$ . Moreover, we let  $\mathbb{F}[X]$  denote the set of all univariate polynomials with coefficients in  $\mathbb{F}$  and indeterminate X, and we let  $\mathbb{F}^{\leq k}[X]$  denote the set of all univariate polynomials of degree at most k. Given a subset  $S \subseteq \mathbb{F}$ , by  $\mathsf{Z}_S(X)$  we denote the *vanishing polynomial on S*, which is defined as  $\mathsf{Z}_S(X) := \prod_{s \in S} (X - s)$ . We say that  $L \subseteq \mathbb{F}^*$  is a multiplicative subgroup if it is closed under multiplication; i.e., if  $x, y \in L$  then  $x \cdot y \in L$ . Given  $\omega \in \mathbb{F}$ , we let  $\langle \omega \rangle := \{1, \omega, \omega^2, \ldots, \omega^{p-1}\}$  denote the multiplicative subgroup of  $\mathbb{F}^*$  generated by  $\omega$ . When  $n = |\langle \omega \rangle|$ , we say that  $\omega$  is a n-th primitive root of unity and sometimes denote such a root of unity as  $\omega_n$ . For any finite set S, we let  $S \subset S$  denote the process of sampling an element of S uniformly and independently at random. Finally, for two finite sets S, we let  $S \subset S$  denote the set of maps of the form  $S \subset S$  denote the given some fixed ordering of  $S \subset S$ , we sometimes view  $S \subset S$  as the vector  $S \subset S$  denote the set of maps of the form  $S \subset S$  denote the given some fixed ordering of  $S \subset S$ , we sometimes view  $S \subset S$  as the vector  $S \subset S$  denote the set of maps of the form  $S \subset S$  denote the set of maps of the form  $S \subset S$  denote the set of maps of the form  $S \subset S$  denote the set of maps of the form  $S \subset S$  denote the set of maps of the form  $S \subset S$  denote the set of maps of the form  $S \subset S$  denote the set of maps of the form  $S \subset S$  denote the set of maps of the form  $S \subset S$  denote the set of maps of the form  $S \subset S$  denote the set of maps of the form  $S \subset S$  denote the set of maps of the form  $S \subset S$  denote the set of maps of the form  $S \subset S$  denote the set of maps of the form  $S \subset S$  denote the set of maps of the form  $S \subset S$  denote the set of all univer
...
A relation  $\mathbf{R}$  is a subset of pairs  $(\mathbb{x}; \mathbb{w}) \in \{0,1\}^* \times \{0,1\}^*$ . The strings  $\mathbb{x}$  are called *inputs, statements*, or *instances*, and the strings  $\mathbb{w}$  are called *witnesses*. To each relation  $\mathbf{R}$  there corresponds a language  $L_{\mathbf{R}} \subseteq \{0,1\}^*$  consisting of all statements  $\mathbb{x}$  such that  $(\mathbb{x},\mathbb{w}) \in \mathbf{R}$  for some  $\mathbb{w}$ . An *indexed relation* is a set of triples  $(\mathbb{i},\mathbb{x};\mathbb{w})$ . Intuitively, the index  $\mathbb{i}$  is fixed at setup time, and each  $\mathbb{i}$  determines a relation  $\mathbf{R}_{\mathbb{i}} := \{(\mathbb{x},\mathbb{w}) \mid (\mathbb{i},\mathbb{x},\mathbb{w}) \in \mathbf{R}\}$ . For example, in the indexed relation of satisfiable arithmetic circuit, the index  $\mathbb{i}$  is a description of a circuit, the statement  $\mathbb{i}$  is the value taken by the "public" wires in the circuit, and the witness  $\mathbb{w}$  is the value taken by the remaining "private" wires.
...
**Definition 3.4** ((Holographic) Interactive Oracle Proofs (hIOP)). A  $\mu$ -round interactive oracle proof for a relation  $\mathbf{R}$  is a tuple of interactive algorithms  $\Pi = (P, V)$  such that:
...
$$\Pr\left[\langle \mathsf{P}(\mathbb{w}), \mathsf{V}\rangle(\mathbb{x}) = 1\right] \geq \delta,$$

where  $\langle P(w), V \rangle(x)$  denotes the output of P and V interacting on common input x where P is additionally given y as input, and the above probability holds over the random coins of y. If y = 1 for all y then the protocol is perfectly complete.

•  $\epsilon$ -Soundness: for any  $x \notin L_R$  and any unbounded interactive algorithm  $P^*$ , we have

$$\Pr\left[\langle \mathsf{P}^*, \mathsf{V} \rangle(x) = 1\right] \le \epsilon,$$
...
*A* μ-round holographic IOP for an indexed relation **R** is a tuple of interactive algorithms  $\Pi = (Ind, P, V)$  such that:
...
- For any fixed i,  $P'(\cdot) := P(Ind(i), \cdot)$ , and  $V'(\cdot) := V^{\llbracket Ind(i) \rrbracket}$ , the tuple (P', V') is a  $\mu(i, x)$ -round IOP for the relation  $\mathbf{R}_i$  with  $\delta(i)$  completeness and  $\epsilon(i)$  soundness.
...
**Definition 3.7** (Honest Verifier Zero-Knowledge). We say that an IOP (P, V) for relation  $\mathbf{R}$  has z-statistical honest-verifier zero-knowledge if there exists a probabilistic polynomial-time simulator algorithm S such that for every  $(\mathbb{X}, \mathbb{W}) \in \mathbf{R}$ , the distribution  $S(\mathbb{X})$  is  $z(\mathbb{X})$ -close to the distribution  $V_{(P(\mathbb{W}),V)(\mathbb{X})}(V(\mathbb{X}))$ . Here, we let  $V_{(P(\mathbb{W}),V)(\mathbb{X})}(V(\mathbb{X}))$  denote the random variable denoting all values the V queries from oracles provided by P, along with any messages from P that V reads in full, during the interaction  $P(\mathbb{W})$ ,  $V(\mathbb{X})$ .
...
$$\mathbf{CoAgg} := \left\{ \begin{pmatrix} \mathbf{i} \\ \mathbf{x} \\ \mathbf{w} \end{pmatrix} = \begin{pmatrix} (\mathbb{F}, D, d, \delta, r) \\ (\mathbb{F}, D)_{i \in [r]} \\ (f_i)_{i \in [r]} \end{pmatrix} \quad \begin{vmatrix} r, \delta \geq 0, \rho = d/|H|, \\ f_i \in \mathbb{F}^H \ \forall i \in [r], \\ (f_i)_{i \in [r]} \ has \ \delta\text{-correlated} \\ agreement \ with \ \mathsf{RS}[\mathbb{F}, H, d] \end{vmatrix} \right\}.$$
...
$$\Pr\left[\left(\mathbb{x},\mathbb{E}^{\tilde{\mathbb{P}}}(\mathbb{x},1^{\mathcal{Q}},1^{\lambda})\right)\in\mathbf{R}\right]\geq\Pr\left[\mathbb{V}^{\rho}(\mathbb{x},\pi)=1\left|\begin{array}{c}\rho\leftarrow\mathcal{U}(\lambda)\\\pi\leftarrow\tilde{\mathbb{P}}^{\rho}\end{array}\right|-\epsilon_{\mathsf{k}}(\mathbb{x},\mathcal{Q},\lambda).\right.$$

Finally, we say that  $(\mathbb{P}, \mathbb{V})$  has z-statistical zero-knowledge (in the explicitly programmable random oracle model) if there exists a probabilistic polynomial-time simulator  $\mathbb{S}$  such that for every  $(\mathbb{X}, \mathbb{W}) \in \mathbf{R}$ ,  $\lambda \in \mathbb{N}$ , and unbounded distinguisher D, the following holds:
...
- round-by-round soundness error  $\varepsilon_{rbr}(x)$ ;
...
then  $(\mathbb{P}, \mathbb{V}) := \mathsf{BCS}^{\mathcal{H}}(\mathsf{P}, \mathsf{V})$  is a non-interactive random oracle proof system for  $\mathbf{R}$  with

- adaptive soundness error  $\varepsilon_{\mathsf{fs}}(\mathbb{X}, Q, \kappa) = Q\varepsilon_{\mathsf{rbr}}(\mathbb{X}) + 3(Q^2 + 1)/2^{\kappa}$  and adaptive knowledge error  $\varepsilon_{\mathsf{fs-k}}(\mathbb{X}, Q, \kappa) = Q\varepsilon_{\mathsf{rbr-k}}(\mathbb{X}) + 3(Q^2 + 1)/2^{\kappa}$  against Q-query adversaries;
...
Moreover, the above results also hold if  ${\bf R}$  is an indexed relation and (Ind, P, V) is a holographic public-coin IOP for  ${\bf R}$  with the same complexities.
...
$$\left(Q_{l}\cdot A+Q_{r}\cdot B+Q_{o}\cdot C+Q_{m}\cdot A\cdot B+Q_{c}\right)\left(X\right)=0$$
...
$$perm-ids^{\sigma}_{\beta,\gamma}(\mathrm{A}(X),\mathrm{B}(X),\mathrm{C}(X),\mathrm{Z}(X))$$
...
• HyperPlonk [CBBZ22]: In Plonk, the prover uses commitments to polynomials q<sub>L</sub>, q<sub>R</sub>, and q<sub>O</sub> (among others) to convince the verifier that these indeed encode a valid computation of the circuit in question C. While in univariate Plonk, the prover's encoding utilizes a cyclic subgroup of F, HyperPlonk instead uses the boolean hypercube for the encoding. This change eliminates the need for an FFT during proof generation and uses a sum-check protocol [LFKN92] instead: shaving off a log(s) (for s being the size of the circuit C) factor in the prover's asymptotic runtime. Furthermore, it also facilitates the use of custom gates of a much higher degree. Any function used multiple times in a circuit can be a custom gate G: F<sup>l</sup> → F. For example, the S-box in a block cipher can be efficiently implemented as a custom gate. A circuit C with a custom gate G can be represented as C[G].
...
$$\varepsilon'(\mathbb{i}) \coloneqq \max \left\{ \frac{\varepsilon_{\mathsf{rbr-s}}(\mathbb{i})}{2\eta\sqrt{\rho}}, \varepsilon_{\mathsf{CA}}(\mathbb{i}_{\mathsf{CA}}) \right\},$$
...
- Suppose that  $\Pi^{\text{OCoAgg}(0)}$  is RBR sound with error  $\varepsilon_{\text{rbr-s}}$ . Then  $\Pi^{\text{OCoAgg}(\delta)}$  has RBR soundness error  $\varepsilon_{\text{rbr-s}}(i)/(2\eta\sqrt{\rho})$ .
...
$$\max\left\{\varepsilon_{\mathsf{rbr-s},\,\delta}(\mathbf{i}),\varepsilon_{\mathsf{CA}}(\mathbf{i}_{\mathsf{CA}})\right\}.$$
...
**Lemma 5.2.** Let  $x \notin \mathcal{L}$ ,  $\mathbb{F}$  be a finite field,  $L_0 \subset \mathbb{F}^*$  be a smooth multiplicative subgroup of size  $2^n$ ,  $d_0 = 2^k$ , and  $\ell \in \mathbb{N}$  be the inputs to the FRI protocol. Let  $m \geq 3$  be an integer and  $\delta, \eta > 0$  such that  $\eta < \sqrt{\rho}/(2m)$  and  $\delta < 1 - \sqrt{\rho} - \eta$ . Let  $G_0: L_0 \to \mathbb{F}$  be  $\delta$ -far from  $\mathsf{RS}^{(0)}$  given to the prover as input. For  $i \in \{1, \ldots, k-1\}$ , let  $G_i \coloneqq H_{x_{i-1}}[G_{i-1}]$  denote the oracle sent by an honest prover P, let  $G_i^*: L_i \to \mathbb{F}$  be some oracle sent by (possibly malicious) prover  $P^*$ , and let  $x_i$  denote the corresponding challenge sent by the verifier during round i of the protocol, with  $G_0^* = G_0$ . Let  $\tau_i \coloneqq \tau_{i-1} \| (G_i^*(z))_{z \in L_i} \| x_i$  for  $L_i \coloneqq \{z^2: z \in L_{i-1}\}$ . Define set  $\mathcal{D}_k$  as follows:

- 1.  $(x, \emptyset) \in \mathcal{D}_k$ ;
- 2.  $(x, \tau_0) \in \mathcal{D}_k$  if and only if  $G_1 = H_{x_0}[G_0]$  is  $\delta$ -far from  $\mathsf{RS}^{(1)}$ ;
...
For i=1, by round-by-round soundness we assume that  $(x,\tau_0)\in\mathcal{D}_k$ . By definition, this implies  $G_1$  is  $\delta$ -far from  $\mathsf{RS}^{(1)}$ . Let  $G_1^*\colon L_1\to\mathbb{F}$  be the oracle sent by  $P^*$  during this round, and let  $x_1\stackrel{s}{\leftarrow}\mathbb{F}$  be the corresponding verifier challenge. Setting  $\tau_1=\tau_0\|(G_1^*(z))_{z\in L_1}\|x_1$ , by definition we know that  $(x,\tau_1)\in\mathcal{D}_k$  if and only if  $G_1^*\neq G_1$  or  $G_2:=H_{x_1}[G_1]$  is  $\delta$ -far from  $\mathsf{RS}^{(2)}$ . Equivalently, we have that  $(x,\tau_1)\notin\mathcal{D}_k$  if and only if  $G_1^*=G_1$  and  $G_2$  is  $\delta$ -close to  $\mathsf{RS}^{(2)}$ . Observe that if  $G_1^*\neq G_1$  then  $(x,\tau_1)\in\mathcal{D}_k$  with probability 1, which implies that  $(x,\tau_1)\notin\mathcal{D}_k$  with probability 0. Thus assume that  $G_1^*=G_1$ . Now by Lemma 5.1, we have

$$\Pr_{x_1 \overset{c}{\leftarrow} \mathbb{F}} [(x,\tau_1) \notin \mathcal{D}_k \mid G_1^* = G_1] \leq \frac{(m+1/2)^7 \cdot |L_0|^2}{3\rho^{3/2} \cdot |\mathbb{F}|},$$
...
- 2. V samples  $\alpha_1, \ldots, \alpha_t \stackrel{s}{\leftarrow} \mathbb{F}$  to P.
...
- 2.  $(x, \tau_{\mathsf{batch}}) \in \mathcal{D}_{\mathsf{batch}}$  if and only if  $G_0 := \sum_i \alpha_i f_i^{(0)}$  is  $\delta$ -far from  $\mathsf{RS}[\mathbb{F}, L_0, d_0]$ .

By definition, we have  $(x, \emptyset) \in \mathcal{D}_{batch}$ , which tells us that at least one  $f_i^{(0)}$  is  $\delta$ -far from  $\mathsf{RS}[\mathbb{F}, L_0, d_0]$ . Now over the sampling of  $\alpha_1, \ldots, \alpha_t \overset{\$}{\leftarrow} \mathbb{F}$ , we want to bound the probability that  $(x, \tau_{\mathsf{batch}}) \notin \mathcal{D}$ ; in other words, we bound the probability that  $G_0$  is  $\delta$ -close to  $\mathsf{RS}^{(0)} := \mathsf{RS}[\mathbb{F}, L_0, d_0]$  given that at least one  $f_i^{(0)}$  is  $\delta$ -far. This probability is exactly given by the following lemma.
...
$$\Pr_{\alpha_1, \dots, \alpha_t \overset{\xi}{\sim} \mathbb{F}} \left[ \Delta(G_0, \mathsf{RS}^{(0)}) < \delta \mid G_0 = \sum_i \alpha_i f_i^{(0)} \right] \leq \frac{(m+1/2)^7 \cdot |L_0|^2}{3\rho^{3/2} \cdot |\mathbb{F}|}.$$
...
$$\Pr_{\alpha \overset{\delta}{\leftarrow} \mathbb{F}} \left[ \Delta(G_0, \mathsf{RS}^{(0)}) < \delta \mid G_0 = \sum_i \alpha^{i-1} f_i^{(0)} \right] \leq \frac{(m+1/2)^7 \cdot |L_0|^2 \cdot (t-1)}{3\rho^{3/2} \cdot |\mathbb{F}|}.$$

Moreover, for  $\delta \leq (1 - \rho)/2$ , we have

$$\Pr_{\substack{\alpha \in \mathbb{F} \\ \alpha \in \mathbb{F}}} \left[ \Delta(G_0, \mathsf{RS}^{(0)}) < \delta \mid G_0 = \sum_{i} \alpha^{i-1} f_i^{(0)} \right] \le \frac{|L_0| \cdot (t-1)}{|\mathbb{F}|}.$$
...
$$\varepsilon_{\mathsf{rbr}-\mathsf{k}}^{\mathsf{FRI}}(\mathbb{F}, L_0, \rho, \delta, m, \ell) \coloneqq \varepsilon_{\mathsf{rbr}}^{\mathsf{FRI}}(\mathbb{F}, L_0, \rho, \delta, m, \ell)$$

for parameters  $\mathbb{F}$ ,  $L_0$ ,  $\rho$ ,  $\delta$ , m,  $\ell$  and  $\varepsilon_{rbr}^{FRI}$  defined in Theorem 4.1. Moreover, for doomed set  $\mathcal{D}'$  defined in Section 5.2, Batched FRI has round-by-round knowledge error
...
$$\Pr_{\substack{x_i \stackrel{\xi}{\sim} \mathbb{F}}} [\Delta(G_i, \mathsf{RS}^{(i)}) \leq \delta \mid \Delta(G_{i-1}, \mathsf{RS}^{(i-1)}) > \delta] > \varepsilon_{\mathsf{rbr}}^{\mathsf{FRI}}.$$
...
- Next the verifier samples  $\alpha \stackrel{\text{s}}{\leftarrow} \mathbb{F}$  and sends it to the prover.
...
- Formulating the SNARG as being built on top of a 0-correlated hIOP  $\Pi^{\mathsf{OCoAgg}(0)}$ ;
- Proving that  $\Pi^{\mathsf{OCoAgg}(0)}$  has RBR soundness and RBR knowledge;
...
- For all  $(i, x, w) \in \mathbf{R}$ :
  - In the last round of interaction between P(Ind(i), x, w) and  $V^{Ind(i),OCoAgg(\delta)}(x)$ , the verifier sends a field element  $\mathfrak{F}$ , uniformly sampled in a subset of  $\mathbb{F}$  or a field extension of  $\mathbb{F}$ , and the honest prover replies with the

values

$$\mathsf{Evals}(\mathbb{X}, \tau, \mathfrak{z}) := (w(\omega^{k_{w,1}}\mathfrak{z}), \dots, w(\omega^{k_{w,n_w}}\mathfrak{z}) \mid w \in \mathit{Words}(\mathbb{X}, \tau)),$$

where  $\tau$  is the transcript so far, and

$$\kappa := \{k_{w,i} \mid w \in Words(\mathbf{x}, \tau), i \in [n_w]\}$$
...
$$\mathsf{quotients}(\mathbf{x},\tau,\mathfrak{z}) := \left\{ \frac{w(X) - w(\omega^{k_{w,j}}\mathfrak{z})}{X - \omega^{k_{w,j}}\mathfrak{z}} \mid w \in \mathit{Words}(\mathbf{x},\tau), j \in [n_w] \right\}$$

have  $\delta$ -correlated agreement in  $RS[\mathbb{F}, D, d-1]$  by using the oracle  $OCoAgg(\delta)$  on the oracles to such maps.
...
$$\varepsilon'(\mathtt{i}) \coloneqq \max \left\{ \frac{\varepsilon_{\mathsf{rbr-s}}(\mathtt{i})}{2\eta\sqrt{\rho}}, \varepsilon_{\mathsf{CA}}(\mathtt{i}_{\mathsf{CA}}) \right\},$$
...
where  $i_{CA}$  has the same meaning as in above.

We prove Theorem 4.6 in two steps. First, given the RBR soundness (knowledge) of  $\Pi^{OCoAgg(0)}$ , we show that the  $\delta$ -correlated hIOP  $\Pi^{OCoAgg(\delta)}$  has RBR soundness (or knowledge) error  $\varepsilon_{rbr-s}(i)/2\eta\sqrt{\rho}$  (or  $\varepsilon_{rbr-k}(i)/2\eta\sqrt{\rho}$ ). Afterward, we use  $\Pi_{CA}$  to compile  $\Pi^{OCoAgg(\delta)}$  into a hIOP with the RBR soundness (and/or RBR knowledge) claimed in the theorem.
...
- Suppose that  $\Pi^{\mathsf{OCoAgg}(0)}$  is RBR sound with error  $\varepsilon_{\mathsf{rbr-s}}$ . Then  $\Pi^{\mathsf{OCoAgg}(\delta)}$  has RBR soundness error  $\varepsilon_{\mathsf{rbr-s}}(\dot{\mathbb{1}})/(2\eta\sqrt{\rho})$ .
...
- All words in  $Words(x, \tau')$  are codewords from  $RS[\mathbb{F}, D, d]$ .
- The words in Words( $x, \tau$ ) have  $\delta$ -correlated agreement RS[F, D, d] with the words in Words( $x, \tau'$ ).

We agree that  $\text{PolyTr}(\mathbb{x}) = \{(\mathbb{x})\}$ . We first prove the claim on the RBR soundness of  $\Pi^{\text{OCoAgg}(\delta)}$ . Assume  $\mathbb{x} \notin \mathcal{L}_{R_i}$ . Let  $\mathcal{A}$  be a malicious prover. Let  $\mathcal{D}_0(i)$  be a doomed set for  $\Pi^{\text{OCoAgg}(0)}$ , with respect to which  $\Pi^{\text{OCoAgg}(0)}$  has RBR soundness  $\varepsilon_{\text{rbr-s}}(i)$ . Define a new doomed set  $\mathcal{D}_{\delta}(i)$  as follows:
...
$$\mathcal{D}_{\delta}(\mathbf{i}) := \left\{ (\mathbf{x}, \tau) \middle| \begin{array}{l} \exists \ i = 0, \dots, \mu(\mathbf{i}, \mathbf{x}) \ \text{ such that } \tau \in \mathrm{PartTr}_{i}(\mathbf{i}, \mathbf{x}), \\ \forall \ (\mathbf{x}, \tau') \in \mathrm{PolyTr}(\mathbf{x}, \tau) \ \text{ we have } (\mathbf{x}, \tau') \in \mathcal{D}_{0}(\mathbf{i}) \end{array} \right\},$$
...
on the set S. Denote the right-hand side polynomial by  $v_{w,j}(X)$ . Notice that since  $|S| \ge (1-\delta)|D|$ , we have  $|S| \ge d$ , since  $(1-\delta)|D| > d$  if and only if  $(1-\delta) = \sqrt{\rho} + \eta > \rho$ . Hence any two polynomials of degree smaller than d agreeing on S must be identical. It follows that for each  $w \in \operatorname{Words}(\mathbb{X},\tau)$  and any two  $j,j' \in [n_w]$ , we have  $v_{w,j}(X) = v_{w,j'}(X)$  as polynomials. Hence for each  $w \in \operatorname{Words}(\mathbb{X},\tau)$  there exists a degree d polynomial d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that d such that

Now notice that, for all  $w \in Words(x, \tau)$  and  $j \in [n_w]$ ,

$$u_w(\omega^{k_{w,j}}\mathfrak{z}) = v_{w,j}(\omega^{k_{w,j}}\mathfrak{z}) = (\omega^{k_{w,j}}\mathfrak{z} - \omega^{k_{w,j}}\mathfrak{z})q_{w,j}(\omega^{k_{w,j}}\mathfrak{z}) + w_i(\omega^{k_{w,j}}\mathfrak{z}) = w_i(\omega^{k_{w,j}}\mathfrak{z}).$$

Hence  $U(x, \tau)$  satisfies our claims.

It follows that if V accepts  $(x, \tau)$ , then  $\operatorname{Words}(x, \tau)$  has  $\delta$ -correlated agreement in  $\operatorname{RS}[\mathbb{F}, H, d]$  on a set S, and that the purported openings  $\operatorname{Evals}(x, \tau, \mathfrak{z})$  sent by  $\mathcal{A}$  are also purported openings of the polynomials in  $U(x, \tau, \mathfrak{z})$ . Let  $\tau'$  be the transcript obtained from  $(x, \tau)$  by replacing  $[\![w]\!]$  with  $[\![u]\!]$ , for all  $w \in \operatorname{Words}(x, \tau)$ . Then the verifier in  $\Pi^{\operatorname{OCoAgg}(\delta)}$  accepts  $(x, \tau')$ , since
...
Next we prove that  $\Pi^{\mathsf{OCoAgg}(\delta)}$  has RBR knowledge with error  $1/2\eta\sqrt{\rho}\varepsilon_{\mathsf{rbr}-\mathsf{k}}(\mathfrak{i})$ . Let  $\mathcal{D}_{0,\mathsf{ks}}$  be a doomed set with respect to which  $\Pi^{\mathsf{OCoAgg}(0)}$  has RBR knowledge with error  $\varepsilon_{\mathsf{rbr}-\mathsf{k}}$ . Let  $\mathsf{Ext}_0$  be the corresponding extractor algorithm. Given an index  $\mathfrak{i}$ , define a doomed set for  $\Pi^{\mathsf{OCoAgg}(\delta)}$  similarly as before:

$$\mathcal{D}_{\delta,\mathsf{ks}}(\mathbf{i}) \coloneqq \left\{ (\mathbf{x},\tau) \middle| \begin{array}{l} \exists \ i = 0, \dots, \mu(\mathbf{i},\mathbf{x}) \ \text{ such that } \tau \in \mathsf{PartTr}_i(\mathbf{i},\mathbf{x}), \\ \forall \ (\mathbf{x},\tau') \in \mathsf{PolyTr}(\mathbf{x},\tau) \ \text{ we have } (\mathbf{x},\tau') \in \mathcal{D}_{0,\mathsf{ks}}(\mathbf{i}) \end{array} \right\},$$

We will prove that  $\Pi^{\mathsf{OCoAgg}(\delta)}$  has RBR knowledge with respect to this set, with the claimed error. To this end, fix an index i and an input x. First note that the exact same arguments as in our RBR analysis yield that all transcripts
...
We claim that then there is  $(x, \tau', m'_i) \in PolyTr(x, \tau, m_i)$  such that
...
- Let  $(i, \mathbb{X}, \mathbb{W}) \in \mathbf{R}$ . Denote  $\mathsf{P}_{\mathsf{compiled}}(\mathsf{Ind}_{\mathsf{compiled}}(i), \mathbb{X}, \mathbb{W})$  by  $\mathsf{P}_{\mathsf{compiled}}$  and  $\mathsf{V}_{\mathsf{compiled}}^{\mathsf{Ind}_{\mathsf{compiled}}(i)}(\mathbb{X})$  by  $\mathsf{V}_{\mathsf{compiled}}$ . We make an analogous notation abuse for the provers and verifiers of  $\mathsf{\Pi}^{\mathsf{OCoAgg}(\delta)}$  and  $\mathsf{\Pi}_{\mathsf{CA}}$ . During the interactive phase:
  ...
  - 3. Next,  $P_{compiled}$  and  $V_{compiled}$  simulate the interactive phase between  $P_{CA}$  and  $V_{CA}$ , with index, input and witness, respectively,  $i_{CA} = (\mathbb{F}, D, d, \delta, |W|)$ ,  $x_{CA} = [W]$ , and  $w_{CA} = W$ .
...
$$\max\left\{\varepsilon_{\mathsf{rbr-s},\delta}(\mathtt{i}),\varepsilon_{\mathsf{CA}}(\mathtt{i}_{\mathsf{CA}})\right\}.$$

• Suppose  $\Pi^{OCoAgg(\delta)}$  has RBR knowledge soundness error  $\varepsilon_{rbr-k,\delta}$ . Then  $\Pi_{compiled}$  has RBR knowledge soundness error

$$\max\left\{ \varepsilon_{\mathsf{rbr}-\mathsf{k},\delta}(\mathtt{i}), \varepsilon_{\mathsf{CA}}(\mathtt{i}_{\mathsf{CA}}) \right\}.$$

*Proof.* Fix an index i and an input x. Let  $\mu(i, x)$  and  $\mu_{CA}(i_{CA}, x_{CA})$  be the number of rounds in the interactive phase of  $\Pi^{OCoAgg(\delta)}$  and in  $\Pi_{CA}$ , respectively. For simplicity, in what follows, we denote these by  $\mu$  and  $\mu_{CA}$ .

Given a partial transcript  $(\mathbb{x},\tau) \in \operatorname{PartTr}_i(\mathbb{i},\mathbb{x})$  for  $\Pi_{\operatorname{compiled}}$ , with  $i > \mu(\mathbb{i},\mathbb{x}) + 1$ , let  $(\mathbb{x},\tau)[: \mu + 2]$  be the partial transcript corresponding to the first  $\mu + 1$  rounds in  $(\mathbb{x},\tau)$ , i.e.  $(\mathbb{x},\tau)[: \mu + 2]$  is the prefix of  $(\mathbb{x},\tau)$  such that  $(\mathbb{x},\tau)[: \mu + 2] \in \operatorname{PartTr}_{\mu+1}(\mathbb{i},\mathbb{x})$ , which corresponds to the messages exchanged during the simulation of  $\Pi^{\operatorname{OCoAgg}(\delta)}$ . Similarly, we let  $(\mathbb{x},\tau)[\mu + 2:]$  be the suffix of  $(\mathbb{x},\tau)$  corresponding to all rounds including and after the round  $\mu + 2$ , so that  $(\mathbb{x},\tau)$  is the concatenation of  $(\mathbb{x},\tau)[: \mu + 2]$  and  $(\mathbb{x},\tau)[\mu + 2:]$ . The transcript  $(\mathbb{x},\tau)[\mu + 2:]$  contains messages exchanged during the simulation of  $\Pi_{\mathsf{CA}}$ .

Assume  $\Pi^{OCoAgg(\delta)}$  is RBR sound with respect to the set  $\mathcal{D}(i)$  and  $\Pi_{CA}$  is RBR sound with respect to the set  $\mathcal{D}_{CA}(i_{CA})$ . We define a new doomed set  $\mathcal{D}_{compiled}(i)$  for  $\Pi_{compiled}$  as follows:
...
More formally, let  $\mathfrak{z}$  be the last nonempty challenge sent by V in the partial transcript  $(\mathfrak{x}, \tau)[: \mu + 2]$  and let Evals( $\mathfrak{z}$ ) be the last message sent by P in  $(\mathfrak{x}, \tau)[: \mu + 2]$ . We say  $(\mathfrak{x}, \tau)$  passes Check 1 of V if the elements Evals( $\mathfrak{z}$ ) are a root to the multivariate polynomial  $F_{\mathfrak{i},\mathfrak{x},\tau}$  from Check 1 in Definition 6.1.
...
Assume  $i > \mu + 2$ . If  $(x, \tau)$  does not pass Check 1 of V, then by definition  $(x, \tau, m_i, c_i)$  also does not pass Check 1. Hence in this case  $P_i = 0$ . Alternatively, if it does pass Check 1 of V, but  $(x_{CA}, (x, \tau)[\mu + 2:]) \in \mathcal{D}_{CA}(i_{CA})$ , then  $P_i \leq \varepsilon_{CA}(i_{CA})$  due to the RBR soundness of  $\Pi_{CA}$ . It remains to analyze the case  $i = \mu + 2$ , which corresponds to the scenario where the last prover message in  $(x, \tau)$  is  $\text{Evals}(\mathfrak{F})$ , and  $m_i = m_{\mu+2}$  is the first message sent by  $P_{CA}$ . By assumption,  $(x, \tau) \in \mathcal{D}_{\text{compiled}}(i)$ , and so by definition of  $\mathcal{D}_{\text{compiled}}$  the verifier V of  $\Pi^{\text{OCoAgg}(\delta)}$  rejects the transcript  $(x, \tau)$ , which is a complete transcript for the protocol  $\Pi^{\text{OCoAgg}(\delta)}$ . Then, by definition of correlated hIOP (Definition 6.1) either  $(x, \tau)$  fails V's Check 1, or it fails Check 2. In the first case, we have  $P_i = 0$  due to the same reasons as before. In the second case, the words  $x_{CA} = W$  do not have  $\delta$ -correlated agreement in RS [F, D, d]. Hence  $x_{CA} \notin \text{CoAgg}(\delta)$ . Consequently,  $(x_{CA}, \emptyset) \in \mathcal{D}_{CA}(i_{CA})$ , and then since  $\Pi_{CA}$  has RBR soundness  $\varepsilon_{CA}(i)$  w.r.t.  $\mathcal{D}_{CA}$ , we obtain that  $(x_{CA}, m_i, c_i) \in \mathcal{D}_{CA}$  except with probability  $\varepsilon_{CA}$ . Then by definition of  $\mathcal{D}_{\text{compiled}}$  we have  $P_i \leq \varepsilon_{CA}(i)$ . Thus  $\Pi_{\text{compiled}}$  has RBR soundness w.r.t. the set  $\mathcal{D}_{\text{compiled}}$ , with error  $\varepsilon_{\text{rbr-s,compiled}} := \max\{\varepsilon_{\text{rbr-s,}\delta}, \varepsilon_{CA}(i_{CA})\}$ .

Now assume  $\Pi^{\mathsf{OCoAgg}(\delta)}$  has RBR knowledge w.r.t. a set  $\mathcal{D}'$ , with error  $\varepsilon_{\mathsf{rbr-k},\delta}$ . We define  $\mathcal{D}'_{\mathsf{compiled}}(\mathtt{i})$  in the exact same manner as  $\mathcal{D}_{\mathsf{compiled}}(\mathtt{i})$ , using  $\mathcal{D}'$  rather than  $\mathcal{D}$ . We will prove that  $\Pi_{\mathsf{compiled}}$  has RBR knowledge w.r.t. this set, with error  $\max \{\varepsilon_{\mathsf{rbr-k},\delta}(\mathtt{i}), \varepsilon_{\mathsf{CA}}(\mathtt{i})\}$ .

Indeed, first notice that if  $(x, \tau) \in \mathcal{D}'_{\text{compiled}}(i)$  is a complete transcript, then  $V_{\text{compiled}}$  rejects  $(x, \tau)$  by the same reasons as in our analysis of the RBR soundness of  $\Pi_{\text{compiled}}$ . Additionally, all transcripts of the form  $(x, \emptyset)$  belong to  $\mathcal{D}'_{\text{compiled}}(i)$  by definition (because they belong to  $\mathcal{D}'(i)$ ), and so the first two clauses in the definition of RBR knowledge hold.

Next, we prove that the third clause also holds. To this end, let Ext be the extractors given by the RBR knowledge of  $\Pi^{\mathsf{OCoAgg}(\delta)}$ . Let  $\varepsilon_{\mathsf{rbr-k},\mathsf{compiled}}(\mathtt{i}) \coloneqq \max\{\varepsilon_{\mathsf{rbr-k},\delta}(\mathtt{i}),\varepsilon_{\mathsf{CA}}(\mathtt{i})\}$ . Let  $i \in [\mu+1+\mu_{\mathsf{CA}}]$ , and let  $\tau \in \mathsf{PartTr}_{i-1}(\mathtt{i},\mathbb{x})$  be such that  $(\mathtt{x},\tau) \in \mathcal{D}'_{\mathsf{compiled}}(\mathtt{i})$ . Define  $m_i$  and  $P_i$  as before, and suppose  $i < \mu+1$  and  $P_i > \varepsilon_{\mathsf{rbr-k},\mathsf{compiled}}(\mathtt{i})$ . Since  $\varepsilon_{\mathsf{rbr-k},\delta}(\mathtt{i})$ , by definition of RBR knowledge, Ext $(\mathtt{i},\mathtt{x},\tau,m_i)$  is a valid witness for the pair  $(\mathtt{i},\mathtt{x})$ .

If  $i = \mu + 1$ , then as we already saw,  $P_i = 0$ , and so there is nothing to analyse in this case. If  $i \geq \mu + 2$ , then by definition of  $\mathcal{D}'_{\mathsf{compiled}}(\mathsf{i})$ , and as we saw previously, we have that either  $(\mathsf{x}, \tau)$  does not pass Check 1 of V, in which case  $P_i = 0$ , or  $\mathsf{x}_{\mathsf{CA}} = [\![W]\!]$  does not have  $\delta$ -correlated agreement in RS $[\![F, D, d]\!]$ . In the latter case, we have  $\Pr_{c_i}[(\mathsf{x}_{\mathsf{CA}}, m_i, c_i) \notin \mathcal{D}_{\mathsf{CA}}] \leq \varepsilon_{\mathsf{CA}}(\mathsf{i}_{\mathsf{CA}})$ , and so  $P_i \leq \varepsilon_{\mathsf{CA}}(\mathsf{i}_{\mathsf{CA}})$  by definition of  $\mathcal{D}'_{\mathsf{compiled}}(\mathsf{i})$ . Hence, for  $i \geq \mu + 2$ , there is never the need to use an extractor because  $P_i \leq \varepsilon_{\mathsf{rbr}-\mathsf{k},\mathsf{compiled}}(\mathsf{i})$ . This completes the proof that  $\mathsf{\Pi}_{\mathsf{compiled}}$  has RBR knowledge with error  $\varepsilon_{\mathsf{rbr}-\mathsf{k},\mathsf{compiled}}(\mathsf{i})$ .
...
For each  $0 \le i \le \mathsf{n} - 1$  and  $0 \le j \le \mathsf{r} - 1$  define vectors  $\boldsymbol{a}, \boldsymbol{b} \in \mathbb{F}^{\mathsf{rn}}$  as

$$\mathbf{a}_{((j-1)n+i)} := a_j(\omega^i), \quad \mathbf{b}_{((j-1)n+i)} := b_j(\omega^i).$$
 (17)
...
We define  $\mathbf{R}_{\mathbf{RPerm}}$  as the following indexed  $(\mathbb{F},D,n)$ -polynomial oracle relation:

$$\begin{split} \mathbf{R}_{\mathbf{RPerm}} \coloneqq & \left\{ \left( (\mathbf{r}, \sigma), (\llbracket a_i \rrbracket, \llbracket b_i \rrbracket)_{i \in [\mathbf{r}]}; (a_i, b_i)_{i \in [\mathbf{r}]} \right) \, \middle| \, \begin{aligned} &(a_1, \ldots, a_\mathbf{r}) = \sigma(b_1, \ldots, b_\mathbf{r}) \\ &a_i, b_i \in \mathsf{RS}[\mathbb{F}, D, \mathsf{n}] \, \textit{for all } i \in [\mathsf{r}] \end{aligned} \right\}. \end{split}$$
...
Before describing a  $\delta$ -correlated hIOP  $\Pi_{\mathbf{R}_{\mathbf{R}\mathbf{Perm}}}(\delta)$  for this relation, we begin by introducing some terminology and notation. Let  $\mathsf{L}_i(X)$  be the i-th element of a Lagrange basis for the elements  $(\omega,\ldots,\omega^{\mathsf{n}})$ , so that  $\mathsf{L}_i(\omega^j)=0$  if  $j\neq i$  and  $\mathsf{L}_i(\omega^i)=1$ . Define two sets of polynomials  $\mathsf{S}_{\mathsf{idj}},\mathsf{S}_{\sigma\mathsf{j}}$ , for  $\mathsf{j}\in[\mathsf{r}]$ , as follows:
...
### 7.1.1 **Description of** $\Pi_{R_{RPerm}}(\delta)$

We now describe the  $\delta$ -correlated hIOP  $\Pi_{R_{RPerm}}(\delta)$  for the relation  $R_{RPerm}$ . To do so, we define a tuple of algorithms  $(\mathsf{Ind}_{R_{RPerm}}, \mathsf{P}_{R_{RPerm}}, \mathsf{V}_{R_{RPerm}}^{\mathsf{OCoAgg}(\delta)})$  as follows. Given a triple  $(\mathtt{i}, \mathtt{x}, \mathtt{w}) \in R_{RPerm}$ :
...
$$f_j(\omega^i) \coloneqq a_j(\omega^i) + \beta \mathsf{S}_{\mathsf{idj}}(\omega^i) + \gamma, \quad g_j(\omega^i) \coloneqq b_j(\omega^i) + \beta \mathsf{S}_{\sigma \mathsf{j}}(\omega^i) + \gamma$$

for all  $i \in [n]$ , and

$$f(X, \beta, \gamma) = f(X) := \prod_{j \in [r]} f_j(X), \quad g(X, \beta, \gamma) = g(X) := \prod_{j \in [r]} g_j(X).$$
 (20)
...
\pi_{j}(\omega^{i}) = \pi_{j-1}(\omega^{i}) \bar{f}_{j}(\omega^{i}) \bar{g}_{j}(\omega^{i})^{-1} \quad \text{for } j = 2, \dots, \mathsf{s} - 1.$$
(24)

P then sends  $[\![z]\!], [\![\pi_1]\!], \dots, [\![\pi_{s-1}]\!]$  to V.
...
Remark 7.2 (Recovering Plonk's original permutation IOP). Plonk's original permutation hIOP can be recovered from  $\Pi_{R_{RPerm}}(\delta)$  by letting  $\delta=0$ , and specializing the parameters r and u to r = 3, and u = 1 (which leads to s = 1).

Here we argue that the protocol  $\Pi_{\mathbf{R}_{\mathbf{RPerm}}}(\delta)$  is a  $\delta$ -correlated hIOP (as per Definition 6.1). Indeed, the only part of the protocol that requires our inspection is the verifier's decision phase. This is because the definition of  $\delta$ -correlated hIOP requires the verifier use the oracle  $\mathsf{OCoAgg}(\delta)$  on a list of words of the form  $(w(X) - a)/(X - \xi)$  for some words w(X) and some field elements  $a, \xi$ . However, in our formulation of  $\Pi_{\mathbf{R}_{\mathbf{RPerm}}}(\delta)$ , the verifier uses  $\mathsf{OCoAgg}(\delta)$  on the list of words  $(\delta_i(X))_{i \in [s]}$ . This is in fact not a discrepancy, since each word  $\delta_i(X)$  can be written in the required manner, e.g. by letting  $\mathsf{Z}_\mathsf{H}(X) = (X - \omega)\mathsf{Z}_\mathsf{H}'(X)$  for some polynomial  $\mathsf{Z}_\mathsf{H}'(X)$  and then writing
...
In this section we describe the  $\delta$ -correlated hIOP OPlonky( $\delta$ ). This is an hIOP for a indexed ( $\mathbb{F}, D, n$ )-polynomial oracle relation  $\mathbf{R}_{ROPlonky}$ , which we formulate next. We let an index i of  $\mathbf{R}_{ROPlonky}$  be a tuple of the form ( $\mathcal{P}, \mathcal{Q}, H, \sigma, Pl, r, r', \ell, t$ ), where  $r, r', \ell, t$  are positive integers, and
...
- $r' \le r$ . Intuitively speaking, execution traces have size  $n \times r$  and r' is the number of columns in it whose entries are subject to "permutation constraints".
...
$$\mathbf{R_{ROPlonky}} := \left\{ (\mathtt{i}, \mathtt{x}, \mathtt{w}) \middle| \begin{array}{l} \mathtt{i} = (\mathcal{P}, \mathcal{Q}, \mathcal{H}, \sigma, \mathsf{PI}, \mathsf{r}, \mathsf{r}', \ell, \mathsf{t}), \\ \mathtt{x} \in \mathbb{F}^{|\mathsf{PI}|}, \ \mathtt{w} \in \mathbb{F}^{\mathsf{rn}}, \\ \mathtt{w} \ \text{satisfies} \ \mathtt{i}, \\ \mathtt{x} = \mathtt{w}^{(\mathsf{pi})}. \end{array} \right\}$$
...
$$\mathsf{Ind}_{\mathsf{Plonky}}(\mathfrak{i}) = (\mathfrak{i}, (\mathsf{sel}_{i}(X))_{i \in [\ell]}, (S_{\mathsf{id}_{i}}(X), S_{\sigma_{i}(X)})_{i \in [r']}, \mathbb{K}, \mathsf{u}, \mathsf{t}),$$
...
The indexer  $\operatorname{Ind}_{R_{\operatorname{RPerm}}}$  is not called since both  $\operatorname{P}_{R_{\operatorname{RPerm}}}$  and  $\operatorname{V}_{R_{\operatorname{RPerm}}}$  already have access to  $\operatorname{Ind}_{R_{\operatorname{RPerm}}}(i') = (i', \mathsf{u}, (S_{\operatorname{idj}}(X), S_{\sigma j(X)})_{j \in [r']})$ . Only the interactive phase is executed, and  $\operatorname{V}_{\operatorname{OPlonky}}$  does not execute the decision phase when the interactive phase is over.

At the end of each of the t executions of the interactive phase, V<sub>OPlonky</sub> has received oracles

$$([\![\mathbf{z}_k]\!],[\![\pi_{k,1}]\!],\ldots,[\![\pi_{k,(\mathsf{s}-1)}]\!])$$
...
**Verifier's challenge 3**  $V_{OPlonky}$  samples a challenge  $\mathfrak{z} \in \mathbb{K} \setminus D$  and sends it to P.

**Prover's message 4** P<sub>OPlonky</sub> replies with the evaluations

$$\left(\mathsf{a}_{j}(\mathfrak{z}),\mathsf{a}_{j}(\omega\mathfrak{z}),\mathsf{q}_{k,j''}(\mathfrak{z}),\pi_{ki}(\mathfrak{z}),\mathsf{z}_{k}(\mathfrak{z}),\mathsf{z}_{k}(\omega\mathfrak{z}),\mathsf{sel}_{j'}(\mathfrak{z}),\mathsf{S}_{\mathsf{idj}}(\mathfrak{z}),\mathsf{S}_{\sigma\mathsf{j}}(\mathfrak{z})\right)$$

for all  $j \in [r], j' \in [\ell], j'' \in [v]k \in [t], i \in [s-1].$ 

### 7.2.4 Verifier's Decision

**Check 1** For each  $k \in [t]$ ,  $V_{OPlonky}$  computes  $Z_H(\mathfrak{z})$ ,  $q_k(\mathfrak{z})$ , and  $d_k(\mathfrak{z})$  with the purported polynomial openings
...
Now that we have described  $\mathsf{OPlonky}(\delta)$ , we can take any  $\mathsf{IOPP}\ \Pi_{\mathsf{CA}}$  for the  $\delta$ -correlated agreement relation  $\mathsf{CoAgg}(\delta)$  (e.g. batched FRI), and compile  $\mathsf{OPlonky}(\delta)$  into a hIOP for the relation  $\mathsf{R}_{\mathsf{ROPlonky}}$  as per  $\mathsf{Definition}\ 6.2$ . Recall that, essentially, this entails replacing the verifier's call to  $\mathsf{OCoAgg}(\delta)$  by an execution of  $\mathsf{\Pi}_{\mathsf{CA}}$ . The resulting hIOP, which we denote by  $\mathsf{Plonky2hIOP}$ , is essentially an abstraction of  $\mathsf{Plonky2}$ 's succinct interactive argument.
...
We fix  $i = (\mathcal{P}, Q, H, \sigma, Pl, r, r', \ell, t)$  to be a valid index for  $\mathbf{R}_{ROPlonkv}$  and  $w \in (\mathbb{F}^n)^r$ .
...
*Proof.* Fix an index  $i = (\mathcal{P}, Q, H, \sigma, Pl, r, r', \ell, t)$ , and potential input x and witnesses  $w \in \mathbb{F}^{rn}$ . For each of the notions of RBR soundness and knowledge we define four sets  $(\mathcal{D}_i^s(i))_{i=0,...,3}$  and  $(\mathcal{D}_i^k(i))_{i=0,...,3}$ . We let:

- $\mathcal{D}_0^s(i)$  consist of all transcripts of the form  $(x, \emptyset)$ , for  $x \notin \mathcal{L}_{R_{ROPlonky},i}$ ,
- $\mathcal{D}_0^k(i)$  consist of all transcripts of the form  $(x, \emptyset)$  for any possible x, including those not in  $\mathcal{L}_{R_{ROPlouky},i}$ .
...
• For some  $k \in [t]$  there do not exist polynomials  $\mathsf{z}(X), \pi_1(X), \dots, \pi_{\mathsf{s}-1}(X)$  of degree at most n with
...
$$\mathcal{D}^{\mathsf{s}}(\mathtt{i}) = \bigcup_{i=0}^{3} \mathcal{D}_{i}^{\mathsf{s}}(\mathtt{i}), \quad \mathcal{D}^{\mathsf{k}}(\mathtt{i}) = \bigcup_{i=0}^{3} \mathcal{D}_{i}^{\mathsf{k}}(\mathtt{i})$$
...
Since  $\mathcal{D}_i^{\mathsf{s}}(\mathtt{i}) = \mathcal{D}_i^{\mathsf{k}}(\mathtt{i})$  for all  $i \in [3]$ , we denote the latter by  $\mathcal{D}_i(\mathtt{i})$ .

Indeed, we begin by analyzing the RBR soundness of  $\mathsf{OPlonky}(0)$ . Let us start with an arbitrary input  $\mathbb{X}$  (not necessarily outside of  $\mathcal{L}_{\mathsf{R}_{\mathsf{ROPlonky}},i}$ . Note first that  $\mathsf{V}_{\mathsf{OPlonky}}$  rejects any complete transcript  $(\mathbb{X},\tau) \in \mathcal{D}_3(i)$ , since we defined  $\mathcal{D}_3(i)$  to consists precisely of those transcripts that fail one of the two checks performed by  $\mathsf{V}_{\mathsf{OPlonky}}$ . Now taking  $i \in [3]$  and  $(\mathbb{X},\tau) \in \mathcal{D}_{i-1}^s(i)$ , define

$$\varepsilon_i(\mathbb{i}) \coloneqq \max_{m_i} \Pr_{c_i}[(\mathbb{x}, \tau, m_i, c_i) \notin \mathcal{D}_i^{\mathsf{s}}(\mathbb{i})].$$

It follows that  $V_{\mathsf{OPlonky}}(0)$  is RBR sound with error  $\varepsilon(i) = \max_{i \in [3]} \{\varepsilon_i(i)\}$ .
...
$$\varepsilon_1(\mathtt{i}) \leq \left(\frac{\mathsf{r'n}}{|\mathbb{F}|}\Pr[\mathcal{E}] + (1 - \Pr[\mathcal{E}])\right)^\mathsf{t},$$
...
$$\max_{m_i} \Pr_{c_i}[(\mathbb{x},\tau,m_i,c_i) \notin \mathcal{D}_i^{\mathsf{k}}(\mathbb{i})] > \varepsilon(\mathbb{i}) \geq \varepsilon_i(\mathbb{i}).$$
...
$$\varepsilon_{\mathsf{rbr}}^{\mathsf{FRI}} \le \frac{1}{Q \cdot 2^{\nu+1}}.$$
(35)

Now Eq. (35) give us an upper bound for  $\varepsilon_{\rm rbr}^{\rm FRI}$ 

Given these constraints, we can now turn to our main goal of setting various values of  $\nu$ ,  $\mathbb{F}$ , k, and  $\rho$ , and analyzing Q and  $\varepsilon_{\rm rhr}^{\rm FRI}$ . For the remainder of this section, we consider the following parameters:
...
We first analyze the above parameters in the context of the provable RBR soundness of FRI given by Theorem 4.1. Before we begin, we first obtain constraints on  $\varepsilon_{rbr}^{FRI}$  under provable security guarantees. Towards this, recall that
...
$$\max\left\{\frac{(m+1/2)^7\cdot |L_0|^2}{3\rho^{3/2}|\mathbb{F}|}, (1-\delta)^\ell\right\} \leq \frac{1}{Q\cdot 2^{\nu+1}}.$$
...
Recall that in FRI, we have  $|L_0|=2^k/\rho$ , so under our fixing of k and  $\rho$ , the parameter  $|L_0|$  is fixed as well. For provable security, we always consider maximal  $\delta$ , i.e., we set  $\delta=1-\sqrt{\rho}(1+1/(2m))$ . Moreover, m is the Johnson proximity parameter and must satisfy  $m\geq 3$ .
...
| v = 20 |                      | Corollary 4.3                                                                                                                  |                                                                                                                               |                                                                                                                        |                                                                                                                        |  |
|--------|----------------------|--------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------|--|
| Q = 2  | 60                   | $\rho = 1/2$                                                                                                                   | $\rho = 1/4$                                                                                                                  | $\rho = 1/8$                                                                                                           | $\rho = 1/16$                                                                                                          |  |
...
| 192    | 10                   | (0.000, 000, 1.00 l f'D)                                                                                                       | (0.500, 100, 500 II:D)                                                                                                        |                                                                                                                        |                                                                                                                        |  |
|        | - 1                  | (0.293, 203, 1.02 MiB)                                                                                                         | (0.500, 102, 590 KiB)                                                                                                         | (0.646, 68, 435 KiB)                                                                                                   | (0.750, 51, 358 KiB)                                                                                                   |  |
|        | 15                   | (0.293, 203, 1.02 MiB)<br>(0.292, 203, 2.00 MiB)                                                                               | (0.500, 102, 590 KiB)<br>(0.499, 102, 1.10 MiB)                                                                               | (0.646, 68, 435 KiB)<br>(0.645, 68, 813 KiB)                                                                           | (0.750, 51, 358 KiB)<br>(0.749, 51, 657 KiB)                                                                           |  |
|        | 15<br>20             |                                                                                                                                |                                                                                                                               |                                                                                                                        |                                                                                                                        |  |
|        |                      | (0.292, 203, 2.00 MiB)                                                                                                         | (0.499, 102, 1.10 MiB)                                                                                                        | (0.645, 68, 813 KiB)                                                                                                   | (0.749, 51, 657 KiB)                                                                                                   |  |
| 256    | 20                   | (0.292, 203, 2.00 MiB)<br>(0.290, 205, 3.32 MiB)                                                                               | (0.499, 102, 1.10 MiB)<br>(0.497, 102, 1.77 MiB)                                                                              | (0.645, 68, 813 KiB)<br>(0.644, 68, 1.27 MiB)                                                                          | (0.749, 51, 657 KiB)<br>(0.747, 51, 1.01 MiB)                                                                          |  |
| 256    | 20<br>25             | (0.292, 203, 2.00 MiB)<br>(0.290, 205, 3.32 MiB)<br>(0.285, 209, 5.02 MiB)                                                     | (0.499, 102, 1.10 MiB)<br>(0.497, 102, 1.77 MiB)<br>(0.492, 104, 2.66 MiB)                                                    | (0.645, 68, 813 KiB)<br>(0.644, 68, 1.27 MiB)<br>(0.639, 69, 1.87 MiB)                                                 | (0.749, 51, 657 KiB)<br>(0.747, 51, 1.01 MiB)<br>(0.742, 52, 1.49 MiB)                                                 |  |
| 256    | 20<br>25<br>10       | (0.292, 203, 2.00 MiB)<br>(0.290, 205, 3.32 MiB)<br>(0.285, 209, 5.02 MiB)<br>(0.293, 203, 1.05 MiB)                           | (0.499, 102, 1.10 MiB)<br>(0.497, 102, 1.77 MiB)<br>(0.492, 104, 2.66 MiB)<br>(0.500, 102, 605 KiB)                           | (0.645, 68, 813 KiB)<br>(0.644, 68, 1.27 MiB)<br>(0.639, 69, 1.87 MiB)<br>(0.646, 68, 446 KiB)                         | (0.749, 51, 657 KiB)<br>(0.747, 51, 1.01 MiB)<br>(0.742, 52, 1.49 MiB)<br>(0.750, 51, 366 KiB)                         |  |
| 256    | 20<br>25<br>10<br>15 | (0.292, 203, 2.00 MiB)<br>(0.290, 205, 3.32 MiB)<br>(0.285, 209, 5.02 MiB)<br>(0.293, 203, 1.05 MiB)<br>(0.293, 203, 2.04 MiB) | (0.499, 102, 1.10 MiB)<br>(0.497, 102, 1.77 MiB)<br>(0.492, 104, 2.66 MiB)<br>(0.500, 102, 605 KiB)<br>(0.500, 102, 1.12 MiB) | (0.645, 68, 813 KiB)<br>(0.644, 68, 1.27 MiB)<br>(0.639, 69, 1.87 MiB)<br>(0.646, 68, 446 KiB)<br>(0.646, 68, 829 KiB) | (0.749, 51, 657 KiB)<br>(0.747, 51, 1.01 MiB)<br>(0.742, 52, 1.49 MiB)<br>(0.750, 51, 366 KiB)<br>(0.750, 51, 669 KiB) |  |
...
|                       | 15  | (0.292, 323, 3.25 MiB) | (0.499, 162, 1.78 MiB) | (0.646, 108, 1.29 MiB) | (0.749, 81, 1.04 MiB) |  |
...
|                     | 20  | (0.500, 102, 1.65 MiB) | (0.750, 51, 909 KiB)  | (0.875, 34, 648 KiB)  | (0.937, 26, 528 KiB) |  |
|                     | 25  | (0.500, 102, 2.45 MiB) | (0.750, 51, 1.30 MiB) | (0.875, 34, 943 KiB)  | (0.937, 26, 762 KiB) |  |
| 256                 | 10  | (0.500, 102, 542 KiB)  | (0.750, 51, 303 KiB)  | (0.875, 34, 223 KiB)  | (0.937, 26, 187 KiB) |  |
...
|        | 10                                                 | (0.500, 101, 505 Kib)                                                                                                                                                                                                               |                                                                                                                                                                                                                                                 |                                                                                                                                                                                              |                                                                                                                                                                                              |  |
|        | 15                                                 | (0.500, 101, 994 KiB)                                                                                                                                                                                                               | (0.750, 51, 550 KiB)                                                                                                                                                                                                                            | (0.875, 34, 398 KiB)                                                                                                                                                                         | (0.938, 26, 329 KiB)                                                                                                                                                                         |  |
|        | - 1                                                | , , , , , , , , , , , , , , , , , , , ,                                                                                                                                                                                             | (0.750, 51, 550 KiB)<br>(0.750, 51, 893 KiB)                                                                                                                                                                                                    | (0.875, 34, 398 KiB)<br>(0.875, 34, 638 KiB)                                                                                                                                                 | (0.938, 26, 329 KiB)<br>(0.938, 26, 520 KiB)                                                                                                                                                 |  |
|        | 15                                                 | (0.500, 101, 994 KiB)                                                                                                                                                                                                               |                                                                                                                                                                                                                                                 |                                                                                                                                                                                              | , , , , , , , , , , , , , , , , , , , ,                                                                                                                                                      |  |
| 192    | 15<br>20                                           | (0.500, 101, 994 KiB)<br>(0.500, 101, 1.60 MiB)                                                                                                                                                                                     | (0.750, 51, 893 KiB)                                                                                                                                                                                                                            | (0.875, 34, 638 KiB)                                                                                                                                                                         | (0.938, 26, 520 KiB)                                                                                                                                                                         |  |
| 192    | 15<br>20<br>25                                     | (0.500, 101, 994 KiB)<br>(0.500, 101, 1.60 MiB)<br>(0.500, 101, 2.39 MiB)                                                                                                                                                           | (0.750, 51, 893 KiB)<br>(0.750, 51, 1.28 MiB)                                                                                                                                                                                                   | (0.875, 34, 638 KiB)<br>(0.875, 34, 930 KiB)                                                                                                                                                 | (0.938, 26, 520 KiB)<br>(0.938, 26, 752 KiB)                                                                                                                                                 |  |
| 192    | 15<br>20<br>25<br>10                               | (0.500, 101, 994 KiB)<br>(0.500, 101, 1.60 MiB)<br>(0.500, 101, 2.39 MiB)<br>(0.500, 101, 521 KiB)                                                                                                                                  | (0.750, 51, 893 KiB)<br>(0.750, 51, 1.28 MiB)<br>(0.750, 51, 295 KiB)                                                                                                                                                                           | (0.875, 34, 638 KiB)<br>(0.875, 34, 930 KiB)<br>(0.875, 34, 218 KiB)                                                                                                                         | (0.938, 26, 520 KiB)<br>(0.938, 26, 752 KiB)<br>(0.938, 26, 183 KiB)                                                                                                                         |  |
| 192    | 15<br>20<br>25<br>10<br>15                         | (0.500, 101, 994 KiB)<br>(0.500, 101, 1.60 MiB)<br>(0.500, 101, 2.39 MiB)<br>(0.500, 101, 521 KiB)<br>(0.500, 101, 1018 KiB)                                                                                                        | (0.750, 51, 893 KiB)<br>(0.750, 51, 1.28 MiB)<br>(0.750, 51, 295 KiB)<br>(0.750, 51, 562 KiB)                                                                                                                                                   | (0.875, 34, 638 KiB)<br>(0.875, 34, 930 KiB)<br>(0.875, 34, 218 KiB)<br>(0.875, 34, 406 KiB)                                                                                                 | (0.938, 26, 520 KiB)<br>(0.938, 26, 752 KiB)<br>(0.938, 26, 183 KiB)<br>(0.938, 26, 335 KiB)                                                                                                 |  |
|        | 15<br>20<br>25<br>10<br>15<br>20                   | (0.500, 101, 994 KiB)<br>(0.500, 101, 1.60 MiB)<br>(0.500, 101, 2.39 MiB)<br>(0.500, 101, 521 KiB)<br>(0.500, 101, 1018 KiB)<br>(0.500, 101, 1.63 MiB)                                                                              | (0.750, 51, 893 KiB)<br>(0.750, 51, 1.28 MiB)<br>(0.750, 51, 295 KiB)<br>(0.750, 51, 562 KiB)<br>(0.750, 51, 909 KiB)                                                                                                                           | (0.875, 34, 638 KiB)<br>(0.875, 34, 930 KiB)<br>(0.875, 34, 218 KiB)<br>(0.875, 34, 406 KiB)<br>(0.875, 34, 648 KiB)                                                                         | (0.938, 26, 520 KiB)<br>(0.938, 26, 752 KiB)<br>(0.938, 26, 183 KiB)<br>(0.938, 26, 335 KiB)<br>(0.938, 26, 528 KiB)                                                                         |  |
|        | 15<br>20<br>25<br>10<br>15<br>20<br>25             | (0.500, 101, 994 KiB)<br>(0.500, 101, 1.60 MiB)<br>(0.500, 101, 2.39 MiB)<br>(0.500, 101, 521 KiB)<br>(0.500, 101, 1018 KiB)<br>(0.500, 101, 1.63 MiB)<br>(0.500, 101, 2.43 MiB)                                                    | (0.750, 51, 893 KiB)<br>(0.750, 51, 1.28 MiB)<br>(0.750, 51, 295 KiB)<br>(0.750, 51, 562 KiB)<br>(0.750, 51, 909 KiB)<br>(0.750, 51, 1.30 MiB)                                                                                                  | (0.875, 34, 638 KiB)<br>(0.875, 34, 930 KiB)<br>(0.875, 34, 218 KiB)<br>(0.875, 34, 406 KiB)<br>(0.875, 34, 648 KiB)<br>(0.875, 34, 943 KiB)                                                 | (0.938, 26, 520 KiB)<br>(0.938, 26, 752 KiB)<br>(0.938, 26, 183 KiB)<br>(0.938, 26, 335 KiB)<br>(0.938, 26, 528 KiB)<br>(0.938, 26, 762 KiB)                                                 |  |
|        | 15<br>20<br>25<br>10<br>15<br>20<br>25<br>10       | (0.500, 101, 994 KiB)<br>(0.500, 101, 1.60 MiB)<br>(0.500, 101, 2.39 MiB)<br>(0.500, 101, 521 KiB)<br>(0.500, 101, 1018 KiB)<br>(0.500, 101, 1.63 MiB)<br>(0.500, 101, 2.43 MiB)<br>(0.500, 101, 536 KiB)                           | (0.750, 51, 893 KiB)<br>(0.750, 51, 1.28 MiB)<br>(0.750, 51, 295 KiB)<br>(0.750, 51, 562 KiB)<br>(0.750, 51, 909 KiB)<br>(0.750, 51, 1.30 MiB)<br>(0.750, 51, 303 KiB)                                                                          | (0.875, 34, 638 KiB)<br>(0.875, 34, 930 KiB)<br>(0.875, 34, 218 KiB)<br>(0.875, 34, 406 KiB)<br>(0.875, 34, 648 KiB)<br>(0.875, 34, 943 KiB)<br>(0.875, 34, 223 KiB)                         | (0.938, 26, 520 KiB)<br>(0.938, 26, 752 KiB)<br>(0.938, 26, 183 KiB)<br>(0.938, 26, 335 KiB)<br>(0.938, 26, 528 KiB)<br>(0.938, 26, 762 KiB)<br>(0.938, 26, 187 KiB)                         |  |
|        | 15<br>20<br>25<br>10<br>15<br>20<br>25<br>10<br>15 | (0.500, 101, 994 KiB)<br>(0.500, 101, 1.60 MiB)<br>(0.500, 101, 2.39 MiB)<br>(0.500, 101, 521 KiB)<br>(0.500, 101, 1018 KiB)<br>(0.500, 101, 1.63 MiB)<br>(0.500, 101, 2.43 MiB)<br>(0.500, 101, 536 KiB)<br>(0.500, 101, 1.02 MiB) | (0.750, 51, 893 KiB)<br>(0.750, 51, 1.28 MiB)<br>(0.750, 51, 295 KiB)<br>(0.750, 51, 562 KiB)<br>(0.750, 51, 909 KiB)<br>(0.750, 51, 1.30 MiB)<br>(0.750, 51, 303 KiB)<br>(0.750, 51, 574 KiB)<br>(0.750, 51, 925 KiB)<br>(0.750, 51, 1.32 MiB) | (0.875, 34, 638 KiB)<br>(0.875, 34, 930 KiB)<br>(0.875, 34, 218 KiB)<br>(0.875, 34, 406 KiB)<br>(0.875, 34, 648 KiB)<br>(0.875, 34, 943 KiB)<br>(0.875, 34, 223 KiB)<br>(0.875, 34, 414 KiB) | (0.938, 26, 520 KiB)<br>(0.938, 26, 752 KiB)<br>(0.938, 26, 183 KiB)<br>(0.938, 26, 335 KiB)<br>(0.938, 26, 528 KiB)<br>(0.938, 26, 762 KiB)<br>(0.938, 26, 187 KiB)<br>(0.938, 26, 341 KiB) |  |
...
|        | 15<br>20                                     | (,                                                                                                                                                                                                                                   | (0.750, 61, 658 KiB)<br>(0.750, 61, 1.04 MiB)                                                                                                                                                                                                      | (0.875, 41, 480 KiB)<br>(0.875, 41, 769 KiB)                                                                                                                                                   | (0.938, 31, 392 KiB)<br>(0.938, 31, 620 KiB)                                                                                                                                                 |  |
|        |                                              | (0.500, 121, 1.16 MiB)                                                                                                                                                                                                               | , , , , , ,                                                                                                                                                                                                                                        |                                                                                                                                                                                                | ` ' '                                                                                                                                                                                        |  |
| 192    | 20                                           | (0.500, 121, 1.16 MiB)<br>(0.500, 121, 1.92 MiB)                                                                                                                                                                                     | (0.750, 61, 1.04 MiB)                                                                                                                                                                                                                              | (0.875, 41, 769 KiB)                                                                                                                                                                           | (0.938, 31, 620 KiB)                                                                                                                                                                         |  |
|        | 20<br>25                                     | (0.500, 121, 1.16 MiB)<br>(0.500, 121, 1.92 MiB)<br>(0.500, 121, 2.86 MiB)                                                                                                                                                           | (0.750, 61, 1.04 MiB)<br>(0.750, 61, 1.54 MiB)                                                                                                                                                                                                     | (0.875, 41, 769 KiB)<br>(0.875, 41, 1.10 MiB)                                                                                                                                                  | (0.938, 31, 620 KiB)<br>(0.938, 31, 896 KiB)                                                                                                                                                 |  |
|        | 20<br>25<br>10                               | (0.500, 121, 1.16 MiB)<br>(0.500, 121, 1.92 MiB)<br>(0.500, 121, 2.86 MiB)<br>(0.500, 121, 624 KiB)                                                                                                                                  | (0.750, 61, 1.04 MiB)<br>(0.750, 61, 1.54 MiB)<br>(0.750, 61, 352 KiB)                                                                                                                                                                             | (0.875, 41, 769 KiB)<br>(0.875, 41, 1.10 MiB)<br>(0.875, 41, 262 KiB)                                                                                                                          | (0.938, 31, 620 KiB)<br>(0.938, 31, 896 KiB)<br>(0.938, 31, 218 KiB)                                                                                                                         |  |
|        | 20<br>25<br>10<br>15                         | (0.500, 121, 1.16 MiB)<br>(0.500, 121, 1.92 MiB)<br>(0.500, 121, 2.86 MiB)<br>(0.500, 121, 624 KiB)<br>(0.500, 121, 1.19 MiB)                                                                                                        | (0.750, 61, 1.04 MiB)<br>(0.750, 61, 1.54 MiB)<br>(0.750, 61, 352 KiB)<br>(0.750, 61, 672 KiB)                                                                                                                                                     | (0.875, 41, 769 KiB)<br>(0.875, 41, 1.10 MiB)<br>(0.875, 41, 262 KiB)<br>(0.875, 41, 490 KiB)                                                                                                  | (0.938, 31, 620 KiB)<br>(0.938, 31, 896 KiB)<br>(0.938, 31, 218 KiB)<br>(0.938, 31, 400 KiB)                                                                                                 |  |
|        | 20<br>25<br>10<br>15<br>20                   | (0.500, 121, 1.16 MiB)<br>(0.500, 121, 1.92 MiB)<br>(0.500, 121, 2.86 MiB)<br>(0.500, 121, 624 KiB)<br>(0.500, 121, 1.19 MiB)<br>(0.500, 121, 1.96 MiB)                                                                              | (0.750, 61, 1.04 MiB)<br>(0.750, 61, 1.54 MiB)<br>(0.750, 61, 352 KiB)<br>(0.750, 61, 672 KiB)<br>(0.750, 61, 1.06 MiB)                                                                                                                            | (0.875, 41, 769 KiB)<br>(0.875, 41, 1.10 MiB)<br>(0.875, 41, 262 KiB)<br>(0.875, 41, 490 KiB)<br>(0.875, 41, 782 KiB)                                                                          | (0.938, 31, 620 KiB)<br>(0.938, 31, 896 KiB)<br>(0.938, 31, 218 KiB)<br>(0.938, 31, 400 KiB)<br>(0.938, 31, 630 KiB)                                                                         |  |
| 192    | 20<br>25<br>10<br>15<br>20<br>25             | (0.500, 121, 1.16 MiB)<br>(0.500, 121, 1.92 MiB)<br>(0.500, 121, 2.86 MiB)<br>(0.500, 121, 624 KiB)<br>(0.500, 121, 1.19 MiB)<br>(0.500, 121, 1.96 MiB)<br>(0.500, 121, 2.91 MiB)                                                    | (0.750, 61, 1.04 MiB)<br>(0.750, 61, 1.54 MiB)<br>(0.750, 61, 352 KiB)<br>(0.750, 61, 672 KiB)<br>(0.750, 61, 1.06 MiB)<br>(0.750, 61, 1.56 MiB)                                                                                                   | (0.875, 41, 769 KiB)<br>(0.875, 41, 1.10 MiB)<br>(0.875, 41, 262 KiB)<br>(0.875, 41, 490 KiB)<br>(0.875, 41, 782 KiB)<br>(0.875, 41, 1.11 MiB)                                                 | (0.938, 31, 620 KiB)<br>(0.938, 31, 896 KiB)<br>(0.938, 31, 218 KiB)<br>(0.938, 31, 400 KiB)<br>(0.938, 31, 630 KiB)<br>(0.938, 31, 909 KiB)                                                 |  |
| 192    | 20<br>25<br>10<br>15<br>20<br>25             | (0.500, 121, 1.16 MiB)<br>(0.500, 121, 1.92 MiB)<br>(0.500, 121, 2.86 MiB)<br>(0.500, 121, 624 KiB)<br>(0.500, 121, 1.19 MiB)<br>(0.500, 121, 1.96 MiB)<br>(0.500, 121, 2.91 MiB)<br>(0.500, 121, 643 KiB)                           | (0.750, 61, 1.04 MiB)<br>(0.750, 61, 1.54 MiB)<br>(0.750, 61, 352 KiB)<br>(0.750, 61, 672 KiB)<br>(0.750, 61, 1.06 MiB)<br>(0.750, 61, 1.56 MiB)<br>(0.750, 61, 362 KiB)                                                                           | (0.875, 41, 769 KiB)<br>(0.875, 41, 1.10 MiB)<br>(0.875, 41, 262 KiB)<br>(0.875, 41, 490 KiB)<br>(0.875, 41, 782 KiB)<br>(0.875, 41, 1.11 MiB)<br>(0.875, 41, 269 KiB)                         | (0.938, 31, 620 KiB)<br>(0.938, 31, 896 KiB)<br>(0.938, 31, 218 KiB)<br>(0.938, 31, 400 KiB)<br>(0.938, 31, 630 KiB)<br>(0.938, 31, 909 KiB)<br>(0.938, 31, 223 KiB)                         |  |
| 192    | 20<br>25<br>10<br>15<br>20<br>25<br>10<br>15 | (0.500, 121, 1.16 MiB)<br>(0.500, 121, 1.92 MiB)<br>(0.500, 121, 2.86 MiB)<br>(0.500, 121, 624 KiB)<br>(0.500, 121, 1.19 MiB)<br>(0.500, 121, 1.96 MiB)<br>(0.500, 121, 2.91 MiB)<br>(0.500, 121, 643 KiB)<br>(0.500, 121, 1.22 MiB) | (0.750, 61, 1.04 MiB)<br>(0.750, 61, 1.54 MiB)<br>(0.750, 61, 352 KiB)<br>(0.750, 61, 672 KiB)<br>(0.750, 61, 1.06 MiB)<br>(0.750, 61, 1.56 MiB)<br>(0.750, 61, 362 KiB)<br>(0.750, 61, 686 KiB)<br>(0.750, 61, 1.08 MiB)<br>(0.750, 61, 1.58 MiB) | (0.875, 41, 769 KiB)<br>(0.875, 41, 1.10 MiB)<br>(0.875, 41, 262 KiB)<br>(0.875, 41, 490 KiB)<br>(0.875, 41, 782 KiB)<br>(0.875, 41, 1.11 MiB)<br>(0.875, 41, 269 KiB)<br>(0.875, 41, 500 KiB) | (0.938, 31, 620 KiB)<br>(0.938, 31, 896 KiB)<br>(0.938, 31, 218 KiB)<br>(0.938, 31, 400 KiB)<br>(0.938, 31, 630 KiB)<br>(0.938, 31, 909 KiB)<br>(0.938, 31, 223 KiB)<br>(0.938, 31, 407 KiB) |  |
...
$$\nu \le \left| \log \left( \frac{1}{2 \cdot Q \cdot \varepsilon_{\mathsf{rbr}}^{\mathsf{FRI}}} \right) \right| \tag{42}$$
...
<span id="page-80-2"></span><sup>13</sup> Note that  $2^{24}$  is a hardcoded upper bound on the polynomial degree  $d_0$ .
...
| Conjecture 5.12 $\eta = \rho/6$ $\delta = 1 - \rho - \eta$                                | ν = 44       | v = 24       | ν = 4        |              | $\delta = 0.70833$ |
| Conjecture 5.14 $\delta = 1 - \rho$                                                       | v = 79       | $\nu = 59$   | $\nu = 39$   | v = 19       | $\delta = 3/4$     |
...
| $\log  \mathbb{F}  = 128,  \rho = 1/2,  k = 31$<br>$\ell = 84,   \pi  = 2.94  \text{MiB}$ | $Q = 2^{20}$ | $Q = 2^{40}$ | $Q = 2^{60}$ | $Q = 2^{80}$ |                    |
|-------------------------------------------------------------------------------------------|--------------|--------------|--------------|--------------|--------------------|
...
| Conjecture 5.14 $\delta = 1 - \rho$                                                   | v = 63       | v = 43       | v = 23       | ν = 3        | $\delta = 7/8$     |
...
vanishes on H. Importantly, each one of these t verifications combines all the polynomials from the t checks of Round 1. This is in contrast to the similar (and arguably more natural, though insecure) approach in which one repeats an entire IOP (or some rounds of it) t times in parallel. In full, such a variation of  $\mathsf{OPlonky}(\delta)$  would proceed as follows (the protocol is essentially the same as the original  $\mathsf{OPlonky}(\delta)$  except for the third prover's message):
...
$$(\mathtt{i}', \mathtt{x}', \mathtt{w}') = \left((\mathtt{r}', \sigma), (\llbracket \mathtt{a}_i \rrbracket, \llbracket \mathtt{a}_i \rrbracket)_{i \in [\mathtt{r}']}, (\mathtt{a}_i, \mathtt{a}_i)_{i \in [\mathtt{r}']}\right)$$
