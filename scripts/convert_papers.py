...
class TailwindRenderer(mistune.HTMLRenderer):
    ...
    def heading(self, text: str, level: int, **attrs: object) -> str:
        if level == 1 and self._skip_h1:
            return ""
        self._sec_counter += 1
        sec_id = f"sec-{self._sec_counter}"
        size = {
        ...
        return (
            f'    <h{level} id="{sec_id}" class="{size}">{text}</h{level}>\n\n'
        )
    ...
    def image(self, text: str, url: str, title: str | None = None) -> str:
        ...
        return f'    <img src="{url}" alt="{alt}" class="my-4 max-w-full" />\n'
...
def create_markdown_parser() -> tuple[mistune.Markdown, TailwindRenderer]:
    ...
    for i, line in enumerate(lines):
        ...
        if m:
            ...
            for j in range(i + 1, min(i + 10, len(lines))):
                ...
                if re.match(
                    r"^(abstract|keywords|date|january|february"
                    r"|march|april|may|june|july|august"
...
def process_paper(
    ...
    # Determine output path: reuse existing filename if force-reconverting,
    ...
    if existing_path is not None and force:
        ...
        if output_path.exists():
            ...
            if f"eprint.iacr.org/{year}/{number}" not in existing_text:
...
def main() -> None:
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Reconvert all papers (ignores existing .astro files)",
    )
    parser.add_argument(
        "--paper",
        type=str,
    ...
    if args.paper:
        ...
        if not re.match(r"^\d{4}_\d+$", paper_id):
            print(
                f"Invalid paper ID: {paper_id} (expected YYYY_NNNN)",
                file=sys.stderr,
            )
            sys.exit(1)
        result = process_paper(
            paper_id, md_parser, renderer, existing_map, force=True
        )
        if result:
